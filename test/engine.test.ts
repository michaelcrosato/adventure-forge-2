...
test("rust durummidas is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durummidas);
  assert.ok(state.visited.includes("rust_durummidas"));
});

test("rust foniomidas is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniomidas);
  assert.ok(state.visited.includes("rust_foniomidas"));
});

test("rust teffmidas is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffmidas);
  assert.ok(state.visited.includes("rust_teffmidas"));
});

test("rust barleymidas is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleymidas);
  assert.ok(state.visited.includes("rust_barleymidas"));
});

test("rust sorghummidas is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghummidas);
  assert.ok(state.visited.includes("rust_sorghummidas"));
});

test("rust milletmidas is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletmidas);
  assert.ok(state.visited.includes("rust_milletmidas"));
});

test("rust cornmidas is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornmidas);
  assert.ok(state.visited.includes("rust_cornmidas"));
});

test("rust oatmidas is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatmidas);
  assert.ok(state.visited.includes("rust_oatmidas"));
});

test("rust durummelchizedek is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durummelchizedek);
  assert.ok(state.visited.includes("rust_durummelchizedek"));
});

test("rust foniomelchizedek is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniomelchizedek);
  assert.ok(state.visited.includes("rust_foniomelchizedek"));
});

test("rust teffmelchizedek is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffmelchizedek);
  assert.ok(state.visited.includes("rust_teffmelchizedek"));
});

test("rust barleymelchizedek is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleymelchizedek);
  assert.ok(state.visited.includes("rust_barleymelchizedek"));
});

test("rust sorghummelchizedek is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghummelchizedek);
  assert.ok(state.visited.includes("rust_sorghummelchizedek"));
});

test("rust milletmelchizedek is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletmelchizedek);
  assert.ok(state.visited.includes("rust_milletmelchizedek"));
});

test("rust cornmelchizedek is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornmelchizedek);
  assert.ok(state.visited.includes("rust_cornmelchizedek"));
});

test("rust oatmelchizedek is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatmelchizedek);
  assert.ok(state.visited.includes("rust_oatmelchizedek"));
});

test("rust durumgoliath is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumgoliath);
  assert.ok(state.visited.includes("rust_durumgoliath"));
});

test("rust foniogoliath is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniogoliath);
  assert.ok(state.visited.includes("rust_foniogoliath"));
});

test("rust teffgoliath is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffgoliath);
  assert.ok(state.visited.includes("rust_teffgoliath"));
});

test("rust barleygoliath is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleygoliath);
  assert.ok(state.visited.includes("rust_barleygoliath"));
});

test("rust sorghumgoliath is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumgoliath);
  assert.ok(state.visited.includes("rust_sorghumgoliath"));
});

test("rust milletgoliath is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletgoliath);
  assert.ok(state.visited.includes("rust_milletgoliath"));
});

test("rust corngoliath is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_corngoliath);
  assert.ok(state.visited.includes("rust_corngoliath"));
});

test("rust oatgoliath is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatgoliath);
  assert.ok(state.visited.includes("rust_oatgoliath"));
});

test("rust durumprimat is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumprimat);
  assert.ok(state.visited.includes("rust_durumprimat"));
});

test("rust fonioprimat is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_fonioprimat);
  assert.ok(state.visited.includes("rust_fonioprimat"));
});

test("rust teffprimat is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffprimat);
  assert.ok(state.visited.includes("rust_teffprimat"));
});

test("rust barleyprimat is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleyprimat);
  assert.ok(state.visited.includes("rust_barleyprimat"));
});

test("rust sorghumprimat is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumprimat);
  assert.ok(state.visited.includes("rust_sorghumprimat"));
});

test("rust milletprimat is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletprimat);
  assert.ok(state.visited.includes("rust_milletprimat"));
});

test("rust cornprimat is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornprimat);
  assert.ok(state.visited.includes("rust_cornprimat"));
});

test("rust oatprimat is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatprimat);
  assert.ok(state.visited.includes("rust_oatprimat"));
});
...
test("rust durumsolomon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumsolomon);
  assert.ok(state.visited.includes("rust_durumsolomon"));
});

test("rust foniosolomon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniosolomon);
  assert.ok(state.visited.includes("rust_foniosolomon"));
});

test("rust teffsolomon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffsolomon);
  assert.ok(state.visited.includes("rust_teffsolomon"));
});

test("rust barleysolomon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleysolomon);
  assert.ok(state.visited.includes("rust_barleysolomon"));
});

test("rust sorghumsolomon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumsolomon);
  assert.ok(state.visited.includes("rust_sorghumsolomon"));
});

test("rust milletsolomon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletsolomon);
  assert.ok(state.visited.includes("rust_milletsolomon"));
});

test("rust cornsolomon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornsolomon);
  assert.ok(state.visited.includes("rust_cornsolomon"));
});

test("rust oatsolomon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatsolomon);
  assert.ok(state.visited.includes("rust_oatsolomon"));
});

test("rust durummelchior is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durummelchior);
  assert.ok(state.visited.includes("rust_durummelchior"));
});

test("rust foniomelchior is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniomelchior);
  assert.ok(state.visited.includes("rust_foniomelchior"));
});

test("rust teffmelchior is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffmelchior);
  assert.ok(state.visited.includes("rust_teffmelchior"));
});

test("rust barleymelchior is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleymelchior);
  assert.ok(state.visited.includes("rust_barleymelchior"));
});

test("rust sorghummelchior is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghummelchior);
  assert.ok(state.visited.includes("rust_sorghummelchior"));
});

test("rust milletmelchior is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletmelchior);
  assert.ok(state.visited.includes("rust_milletmelchior"));
});

test("rust cornmelchior is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornmelchior);
  assert.ok(state.visited.includes("rust_cornmelchior"));
});

test("rust oatmelchior is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatmelchior);
  assert.ok(state.visited.includes("rust_oatmelchior"));
});

test("rust durumnebuchadnezzar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumnebuchadnezzar);
  assert.ok(state.visited.includes("rust_durumnebuchadnezzar"));
});

test("rust fonionebuchadnezzar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_fonionebuchadnezzar);
  assert.ok(state.visited.includes("rust_fonionebuchadnezzar"));
});

test("rust teffnebuchadnezzar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffnebuchadnezzar);
  assert.ok(state.visited.includes("rust_teffnebuchadnezzar"));
});

test("rust barleynebuchadnezzar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleynebuchadnezzar);
  assert.ok(state.visited.includes("rust_barleynebuchadnezzar"));
});

test("rust sorghumnebuchadnezzar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumnebuchadnezzar);
  assert.ok(state.visited.includes("rust_sorghumnebuchadnezzar"));
});

test("rust milletnebuchadnezzar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletnebuchadnezzar);
  assert.ok(state.visited.includes("rust_milletnebuchadnezzar"));
});

test("rust cornnebuchadnezzar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornnebuchadnezzar);
  assert.ok(state.visited.includes("rust_cornnebuchadnezzar"));
});

test("rust oatnebuchadnezzar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatnebuchadnezzar);
  assert.ok(state.visited.includes("rust_oatnebuchadnezzar"));
});

test("rust durumbalthazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumbalthazar);
  assert.ok(state.visited.includes("rust_durumbalthazar"));
});

test("rust foniobalthazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniobalthazar);
  assert.ok(state.visited.includes("rust_foniobalthazar"));
});

test("rust teffbalthazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffbalthazar);
  assert.ok(state.visited.includes("rust_teffbalthazar"));
});

test("rust barleybalthazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleybalthazar);
  assert.ok(state.visited.includes("rust_barleybalthazar"));
});

test("rust sorghumbalthazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumbalthazar);
  assert.ok(state.visited.includes("rust_sorghumbalthazar"));
});

test("rust milletbalthazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletbalthazar);
  assert.ok(state.visited.includes("rust_milletbalthazar"));
});

test("rust cornbalthazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornbalthazar);
  assert.ok(state.visited.includes("rust_cornbalthazar"));
});

test("rust oatbalthazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatbalthazar);
  assert.ok(state.visited.includes("rust_oatbalthazar"));
});

test("rust durumsalmanazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumsalmanazar);
  assert.ok(state.visited.includes("rust_durumsalmanazar"));
});

test("rust foniosalmanazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniosalmanazar);
  assert.ok(state.visited.includes("rust_foniosalmanazar"));
});

test("rust teffsalmanazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffsalmanazar);
  assert.ok(state.visited.includes("rust_teffsalmanazar"));
});

test("rust barleysalmanazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleysalmanazar);
  assert.ok(state.visited.includes("rust_barleysalmanazar"));
});

test("rust sorghumsalmanazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumsalmanazar);
  assert.ok(state.visited.includes("rust_sorghumsalmanazar"));
});

test("rust milletsalmanazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletsalmanazar);
  assert.ok(state.visited.includes("rust_milletsalmanazar"));
});

test("rust cornsalmanazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornsalmanazar);
  assert.ok(state.visited.includes("rust_cornsalmanazar"));
});

test("rust oatsalmanazar is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatsalmanazar);
  assert.ok(state.visited.includes("rust_oatsalmanazar"));
});

test("rust durummethuselah is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durummethuselah);
  assert.ok(state.visited.includes("rust_durummethuselah"));
});

test("rust foniomethuselah is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniomethuselah);
  assert.ok(state.visited.includes("rust_foniomethuselah"));
});

test("rust teffmethuselah is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffmethuselah);
  assert.ok(state.visited.includes("rust_teffmethuselah"));
});

test("rust barleymethuselah is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleymethuselah);
  assert.ok(state.visited.includes("rust_barleymethuselah"));
});

test("rust sorghummethuselah is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghummethuselah);
  assert.ok(state.visited.includes("rust_sorghummethuselah"));
});

test("rust milletmethuselah is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletmethuselah);
  assert.ok(state.visited.includes("rust_milletmethuselah"));
});

test("rust cornmethuselah is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornmethuselah);
  assert.ok(state.visited.includes("rust_cornmethuselah"));
});

test("rust oatmethuselah is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatmethuselah);
  assert.ok(state.visited.includes("rust_oatmethuselah"));
});

test("rust durumrehoboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumrehoboam);
  assert.ok(state.visited.includes("rust_durumrehoboam"));
});

test("rust foniorehoboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniorehoboam);
  assert.ok(state.visited.includes("rust_foniorehoboam"));
});

test("rust teffrehoboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffrehoboam);
  assert.ok(state.visited.includes("rust_teffrehoboam"));
});

test("rust barleyrehoboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleyrehoboam);
  assert.ok(state.visited.includes("rust_barleyrehoboam"));
});

test("rust sorghumrehoboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumrehoboam);
  assert.ok(state.visited.includes("rust_sorghumrehoboam"));
});

test("rust milletrehoboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletrehoboam);
  assert.ok(state.visited.includes("rust_milletrehoboam"));
});

test("rust cornrehoboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornrehoboam);
  assert.ok(state.visited.includes("rust_cornrehoboam"));
});

test("rust oatrehoboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatrehoboam);
  assert.ok(state.visited.includes("rust_oatrehoboam"));
});

test("rust durumjeroboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumjeroboam);
  assert.ok(state.visited.includes("rust_durumjeroboam"));
});

test("rust foniojeroboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniojeroboam);
  assert.ok(state.visited.includes("rust_foniojeroboam"));
});

test("rust teffjeroboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffjeroboam);
  assert.ok(state.visited.includes("rust_teffjeroboam"));
});

test("rust barleyjeroboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleyjeroboam);
  assert.ok(state.visited.includes("rust_barleyjeroboam"));
});

test("rust sorghumjeroboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumjeroboam);
  assert.ok(state.visited.includes("rust_sorghumjeroboam"));
});

test("rust milletjeroboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletjeroboam);
  assert.ok(state.visited.includes("rust_milletjeroboam"));
});

test("rust cornjeroboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornjeroboam);
  assert.ok(state.visited.includes("rust_cornjeroboam"));
});

test("rust oatjeroboam is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatjeroboam);
  assert.ok(state.visited.includes("rust_oatjeroboam"));
});

test("rust durummagnum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durummagnum);
  assert.ok(state.visited.includes("rust_durummagnum"));
});

test("rust foniomagnum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniomagnum);
  assert.ok(state.visited.includes("rust_foniomagnum"));
});

test("rust teffmagnum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffmagnum);
  assert.ok(state.visited.includes("rust_teffmagnum"));
});

test("rust barleymagnum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleymagnum);
  assert.ok(state.visited.includes("rust_barleymagnum"));
});

test("rust sorghummagnum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghummagnum);
  assert.ok(state.visited.includes("rust_sorghummagnum"));
});

test("rust milletmagnum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletmagnum);
  assert.ok(state.visited.includes("rust_milletmagnum"));
});

test("rust cornmagnum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornmagnum);
  assert.ok(state.visited.includes("rust_cornmagnum"));
});

test("rust oatmagnum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatmagnum);
  assert.ok(state.visited.includes("rust_oatmagnum"));
});

test("rust durumgallon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumgallon);
  assert.ok(state.visited.includes("rust_durumgallon"));
});

test("rust foniogallon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniogallon);
  assert.ok(state.visited.includes("rust_foniogallon"));
});

test("rust teffgallon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffgallon);
  assert.ok(state.visited.includes("rust_teffgallon"));
});

test("rust barleygallon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleygallon);
  assert.ok(state.visited.includes("rust_barleygallon"));
});

test("rust sorghumgallon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumgallon);
  assert.ok(state.visited.includes("rust_sorghumgallon"));
});

test("rust milletgallon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletgallon);
  assert.ok(state.visited.includes("rust_milletgallon"));
});

test("rust corngallon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_corngallon);
  assert.ok(state.visited.includes("rust_corngallon"));
});

test("rust oatgallon is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatgallon);
  assert.ok(state.visited.includes("rust_oatgallon"));
});

test("rust durumquart is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumquart);
  assert.ok(state.visited.includes("rust_durumquart"));
});

test("rust fonioquart is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_fonioquart);
  assert.ok(state.visited.includes("rust_fonioquart"));
});

test("rust teffquart is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffquart);
  assert.ok(state.visited.includes("rust_teffquart"));
});

test("rust barleyquart is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleyquart);
  assert.ok(state.visited.includes("rust_barleyquart"));
});

test("rust sorghumquart is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumquart);
  assert.ok(state.visited.includes("rust_sorghumquart"));
});

test("rust milletquart is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletquart);
  assert.ok(state.visited.includes("rust_milletquart"));
});

test("rust cornquart is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornquart);
  assert.ok(state.visited.includes("rust_cornquart"));
});

test("rust oatquart is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatquart);
  assert.ok(state.visited.includes("rust_oatquart"));
});

test("rust durumpint is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumpint);
  assert.ok(state.visited.includes("rust_durumpint"));
});

test("rust foniopint is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniopint);
  assert.ok(state.visited.includes("rust_foniopint"));
});

test("rust teffpint is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffpint);
  assert.ok(state.visited.includes("rust_teffpint"));
});

test("rust barleypint is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleypint);
  assert.ok(state.visited.includes("rust_barleypint"));
});

test("rust sorghumpint is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumpint);
  assert.ok(state.visited.includes("rust_sorghumpint"));
});

test("rust milletpint is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletpint);
  assert.ok(state.visited.includes("rust_milletpint"));
});

test("rust cornpint is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornpint);
  assert.ok(state.visited.includes("rust_cornpint"));
});

test("rust oatpint is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatpint);
  assert.ok(state.visited.includes("rust_oatpint"));
});

test("rust durumgill is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumgill);
  assert.ok(state.visited.includes("rust_durumgill"));
});

test("rust foniogill is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniogill);
  assert.ok(state.visited.includes("rust_foniogill"));
});

test("rust teffgill is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffgill);
  assert.ok(state.visited.includes("rust_teffgill"));
});

test("rust barleygill is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleygill);
  assert.ok(state.visited.includes("rust_barleygill"));
});

test("rust sorghumgill is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumgill);
  assert.ok(state.visited.includes("rust_sorghumgill"));
});

test("rust milletgill is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletgill);
  assert.ok(state.visited.includes("rust_milletgill"));
});

test("rust corngill is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_corngill);
  assert.ok(state.visited.includes("rust_corngill"));
});

test("rust oatgill is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatgill);
  assert.ok(state.visited.includes("rust_oatgill"));
});

test("rust durumnoggin is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumnoggin);
  assert.ok(state.visited.includes("rust_durumnoggin"));
});

test("rust fonionoggin is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_fonionoggin);
  assert.ok(state.visited.includes("rust_fonionoggin"));
});

test("rust teffnoggin is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffnoggin);
  assert.ok(state.visited.includes("rust_teffnoggin"));
});

test("rust barleynoggin is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleynoggin);
  assert.ok(state.visited.includes("rust_barleynoggin"));
});

test("rust sorghumnoggin is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumnoggin);
  assert.ok(state.visited.includes("rust_sorghumnoggin"));
});

test("rust milletnoggin is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletnoggin);
  assert.ok(state.visited.includes("rust_milletnoggin"));
});

test("rust cornnoggin is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornnoggin);
  assert.ok(state.visited.includes("rust_cornnoggin"));
});

test("rust oatnoggin is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatnoggin);
  assert.ok(state.visited.includes("rust_oatnoggin"));
});

test("rust durumjorum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumjorum);
  assert.ok(state.visited.includes("rust_durumjorum"));
});

test("rust foniojorum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniojorum);
  assert.ok(state.visited.includes("rust_foniojorum"));
});

test("rust teffjorum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffjorum);
  assert.ok(state.visited.includes("rust_teffjorum"));
});

test("rust barleyjorum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleyjorum);
  assert.ok(state.visited.includes("rust_barleyjorum"));
});

test("rust sorghumjorum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumjorum);
  assert.ok(state.visited.includes("rust_sorghumjorum"));
});

test("rust milletjorum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletjorum);
  assert.ok(state.visited.includes("rust_milletjorum"));
});

test("rust cornjorum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornjorum);
  assert.ok(state.visited.includes("rust_cornjorum"));
});

test("rust oatjorum is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatjorum);
  assert.ok(state.visited.includes("rust_oatjorum"));
});

test("rust durumstoup is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumstoup);
  assert.ok(state.visited.includes("rust_durumstoup"));
});

test("rust foniostoup is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniostoup);
  assert.ok(state.visited.includes("rust_foniostoup"));
});

test("rust teffstoup is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffstoup);
  assert.ok(state.visited.includes("rust_teffstoup"));
});

test("rust barleystoup is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleystoup);
  assert.ok(state.visited.includes("rust_barleystoup"));
});

test("rust sorghumstoup is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumstoup);
  assert.ok(state.visited.includes("rust_sorghumstoup"));
});

test("rust milletstoup is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletstoup);
  assert.ok(state.visited.includes("rust_milletstoup"));
});

test("rust cornstoup is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornstoup);
  assert.ok(state.visited.includes("rust_cornstoup"));
});

test("rust oatstoup is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatstoup);
  assert.ok(state.visited.includes("rust_oatstoup"));
});

test("rust durumtumbler is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumtumbler);
  assert.ok(state.visited.includes("rust_durumtumbler"));
});

test("rust foniotumbler is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniotumbler);
  assert.ok(state.visited.includes("rust_foniotumbler"));
});

test("rust tefftumbler is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_tefftumbler);
  assert.ok(state.visited.includes("rust_tefftumbler"));
});

test("rust barleytumbler is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleytumbler);
  assert.ok(state.visited.includes("rust_barleytumbler"));
});

test("rust sorghumtumbler is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumtumbler);
  assert.ok(state.visited.includes("rust_sorghumtumbler"));
});

test("rust millettumbler is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_millettumbler);
  assert.ok(state.visited.includes("rust_millettumbler"));
});

test("rust corntumbler is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_corntumbler);
  assert.ok(state.visited.includes("rust_corntumbler"));
});

test("rust oattumbler is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oattumbler);
  assert.ok(state.visited.includes("rust_oattumbler"));
});

test("rust durumbeaker is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumbeaker);
  assert.ok(state.visited.includes("rust_durumbeaker"));
});

test("rust foniobeaker is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniobeaker);
  assert.ok(state.visited.includes("rust_foniobeaker"));
});

test("rust teffbeaker is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffbeaker);
  assert.ok(state.visited.includes("rust_teffbeaker"));
});

test("rust barleybeaker is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleybeaker);
  assert.ok(state.visited.includes("rust_barleybeaker"));
});

test("rust sorghumbeaker is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumbeaker);
  assert.ok(state.visited.includes("rust_sorghumbeaker"));
});

test("rust milletbeaker is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletbeaker);
  assert.ok(state.visited.includes("rust_milletbeaker"));
});

test("rust cornbeaker is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornbeaker);
  assert.ok(state.visited.includes("rust_cornbeaker"));
});

test("rust oatbeaker is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatbeaker);
  assert.ok(state.visited.includes("rust_oatbeaker"));
});

test("rust durumstein is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumstein);
  assert.ok(state.visited.includes("rust_durumstein"));
});

test("rust foniostein is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniostein);
  assert.ok(state.visited.includes("rust_foniostein"));
});

test("rust teffstein is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffstein);
  assert.ok(state.visited.includes("rust_teffstein"));
});

test("rust barleystein is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleystein);
  assert.ok(state.visited.includes("rust_barleystein"));
});

test("rust sorghumstein is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumstein);
  assert.ok(state.visited.includes("rust_sorghumstein"));
});

test("rust milletstein is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletstein);
  assert.ok(state.visited.includes("rust_milletstein"));
});

test("rust cornstein is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornstein);
  assert.ok(state.visited.includes("rust_cornstein"));
});

test("rust oatstein is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatstein);
  assert.ok(state.visited.includes("rust_oatstein"));
});

test("rust durummug is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durummug);
  assert.ok(state.visited.includes("rust_durummug"));
});

test("rust foniomug is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniomug);
  assert.ok(state.visited.includes("rust_foniomug"));
});

test("rust teffmug is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffmug);
  assert.ok(state.visited.includes("rust_teffmug"));
});

test("rust barleymug is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleymug);
  assert.ok(state.visited.includes("rust_barleymug"));
});

test("rust sorghummug is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghummug);
  assert.ok(state.visited.includes("rust_sorghummug"));
});

test("rust milletmug is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletmug);
  assert.ok(state.visited.includes("rust_milletmug"));
});

test("rust cornmug is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_cornmug);
  assert.ok(state.visited.includes("rust_cornmug"));
});

test("rust oatmug is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatmug);
  assert.ok(state.visited.includes("rust_oatmug"));
});

test("rust durumgoblet is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumgoblet);
  assert.ok(state.visited.includes("rust_durumgoblet"));
});

test("rust foniogoblet is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniogoblet);
  assert.ok(state.visited.includes("rust_foniogoblet"));
});

test("rust teffgoblet is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_teffgoblet);
  assert.ok(state.visited.includes("rust_teffgoblet"));
});

test("rust barleygoblet is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleygoblet);
  assert.ok(state.visited.includes("rust_barleygoblet"));
});

test("rust sorghumgoblet is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumgoblet);
  assert.ok(state.visited.includes("rust_sorghumgoblet"));
});

test("rust milletgoblet is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_milletgoblet);
  assert.ok(state.visited.includes("rust_milletgoblet"));
});

test("rust corngoblet is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_corngoblet);
  assert.ok(state.visited.includes("rust_corngoblet"));
});

test("rust oatgoblet is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oatgoblet);
  assert.ok(state.visited.includes("rust_oatgoblet"));
});

test("rust durumtankard is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_durumtankard);
  assert.ok(state.visited.includes("rust_durumtankard"));
});

test("rust foniotankard is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_foniotankard);
  assert.ok(state.visited.includes("rust_foniotankard"));
});

test("rust tefftankard is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_tefftankard);
  assert.ok(state.visited.includes("rust_tefftankard"));
});

test("rust barleytankard is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_barleytankard);
  assert.ok(state.visited.includes("rust_barleytankard"));
});

test("rust sorghumtankard is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_sorghumtankard);
  assert.ok(state.visited.includes("rust_sorghumtankard"));
});

test("rust millettankard is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_millettankard);
  assert.ok(state.visited.includes("rust_millettankard"));
});

test("rust corntankard is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_corntankard);
  assert.ok(state.visited.includes("rust_corntankard"));
});

test("rust oattankard is scored on the proven path", () => {
  const { state } = playWalkthrough(1);
  assert.equal(state.ended?.id, "beacon_lit");
  assert.ok(state.flags.did_pinch_oattankard);
  assert.ok(state.visited.includes("rust_oattankard"));
});
