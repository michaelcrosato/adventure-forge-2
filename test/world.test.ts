...
test("walkthrough includes pinching the rust-durumgallon durumgallon", () => {
  assert.ok(world.rooms.rust_durumgallon, "rust_durumgallon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumgallon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniogallon foniogallon", () => {
  assert.ok(world.rooms.rust_foniogallon, "rust_foniogallon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniogallon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffgallon teffgallon", () => {
  assert.ok(world.rooms.rust_teffgallon, "rust_teffgallon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffgallon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleygallon barleygallon", () => {
  assert.ok(world.rooms.rust_barleygallon, "rust_barleygallon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleygallon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumgallon sorghumgallon", () => {
  assert.ok(world.rooms.rust_sorghumgallon, "rust_sorghumgallon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumgallon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletgallon milletgallon", () => {
  assert.ok(world.rooms.rust_milletgallon, "rust_milletgallon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletgallon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corngallon corngallon", () => {
  assert.ok(world.rooms.rust_corngallon, "rust_corngallon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corngallon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatgallon oatgallon", () => {
  assert.ok(world.rooms.rust_oatgallon, "rust_oatgallon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatgallon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumquart durumquart", () => {
  assert.ok(world.rooms.rust_durumquart, "rust_durumquart room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumquart"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioquart fonioquart", () => {
  assert.ok(world.rooms.rust_fonioquart, "rust_fonioquart room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioquart"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffquart teffquart", () => {
  assert.ok(world.rooms.rust_teffquart, "rust_teffquart room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffquart"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyquart barleyquart", () => {
  assert.ok(world.rooms.rust_barleyquart, "rust_barleyquart room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyquart"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumquart sorghumquart", () => {
  assert.ok(world.rooms.rust_sorghumquart, "rust_sorghumquart room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumquart"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletquart milletquart", () => {
  assert.ok(world.rooms.rust_milletquart, "rust_milletquart room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletquart"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornquart cornquart", () => {
  assert.ok(world.rooms.rust_cornquart, "rust_cornquart room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornquart"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatquart oatquart", () => {
  assert.ok(world.rooms.rust_oatquart, "rust_oatquart room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatquart"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumpint durumpint", () => {
  assert.ok(world.rooms.rust_durumpint, "rust_durumpint room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumpint"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniopint foniopint", () => {
  assert.ok(world.rooms.rust_foniopint, "rust_foniopint room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniopint"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffpint teffpint", () => {
  assert.ok(world.rooms.rust_teffpint, "rust_teffpint room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffpint"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleypint barleypint", () => {
  assert.ok(world.rooms.rust_barleypint, "rust_barleypint room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleypint"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumpint sorghumpint", () => {
  assert.ok(world.rooms.rust_sorghumpint, "rust_sorghumpint room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumpint"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletpint milletpint", () => {
  assert.ok(world.rooms.rust_milletpint, "rust_milletpint room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletpint"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornpint cornpint", () => {
  assert.ok(world.rooms.rust_cornpint, "rust_cornpint room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornpint"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatpint oatpint", () => {
  assert.ok(world.rooms.rust_oatpint, "rust_oatpint room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatpint"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumgill durumgill", () => {
  assert.ok(world.rooms.rust_durumgill, "rust_durumgill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumgill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniogill foniogill", () => {
  assert.ok(world.rooms.rust_foniogill, "rust_foniogill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniogill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffgill teffgill", () => {
  assert.ok(world.rooms.rust_teffgill, "rust_teffgill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffgill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleygill barleygill", () => {
  assert.ok(world.rooms.rust_barleygill, "rust_barleygill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleygill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumgill sorghumgill", () => {
  assert.ok(world.rooms.rust_sorghumgill, "rust_sorghumgill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumgill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletgill milletgill", () => {
  assert.ok(world.rooms.rust_milletgill, "rust_milletgill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletgill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corngill corngill", () => {
  assert.ok(world.rooms.rust_corngill, "rust_corngill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corngill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatgill oatgill", () => {
  assert.ok(world.rooms.rust_oatgill, "rust_oatgill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatgill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumnoggin durumnoggin", () => {
  assert.ok(world.rooms.rust_durumnoggin, "rust_durumnoggin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumnoggin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonionoggin fonionoggin", () => {
  assert.ok(world.rooms.rust_fonionoggin, "rust_fonionoggin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonionoggin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffnoggin teffnoggin", () => {
  assert.ok(world.rooms.rust_teffnoggin, "rust_teffnoggin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffnoggin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleynoggin barleynoggin", () => {
  assert.ok(world.rooms.rust_barleynoggin, "rust_barleynoggin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleynoggin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumnoggin sorghumnoggin", () => {
  assert.ok(world.rooms.rust_sorghumnoggin, "rust_sorghumnoggin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumnoggin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletnoggin milletnoggin", () => {
  assert.ok(world.rooms.rust_milletnoggin, "rust_milletnoggin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletnoggin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornnoggin cornnoggin", () => {
  assert.ok(world.rooms.rust_cornnoggin, "rust_cornnoggin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornnoggin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatnoggin oatnoggin", () => {
  assert.ok(world.rooms.rust_oatnoggin, "rust_oatnoggin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatnoggin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumjorum durumjorum", () => {
  assert.ok(world.rooms.rust_durumjorum, "rust_durumjorum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumjorum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniojorum foniojorum", () => {
  assert.ok(world.rooms.rust_foniojorum, "rust_foniojorum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniojorum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffjorum teffjorum", () => {
  assert.ok(world.rooms.rust_teffjorum, "rust_teffjorum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffjorum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyjorum barleyjorum", () => {
  assert.ok(world.rooms.rust_barleyjorum, "rust_barleyjorum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyjorum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumjorum sorghumjorum", () => {
  assert.ok(world.rooms.rust_sorghumjorum, "rust_sorghumjorum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumjorum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletjorum milletjorum", () => {
  assert.ok(world.rooms.rust_milletjorum, "rust_milletjorum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletjorum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornjorum cornjorum", () => {
  assert.ok(world.rooms.rust_cornjorum, "rust_cornjorum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornjorum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatjorum oatjorum", () => {
  assert.ok(world.rooms.rust_oatjorum, "rust_oatjorum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatjorum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumstoup durumstoup", () => {
  assert.ok(world.rooms.rust_durumstoup, "rust_durumstoup room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumstoup"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniostoup foniostoup", () => {
  assert.ok(world.rooms.rust_foniostoup, "rust_foniostoup room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniostoup"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffstoup teffstoup", () => {
  assert.ok(world.rooms.rust_teffstoup, "rust_teffstoup room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffstoup"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleystoup barleystoup", () => {
  assert.ok(world.rooms.rust_barleystoup, "rust_barleystoup room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleystoup"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumstoup sorghumstoup", () => {
  assert.ok(world.rooms.rust_sorghumstoup, "rust_sorghumstoup room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumstoup"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletstoup milletstoup", () => {
  assert.ok(world.rooms.rust_milletstoup, "rust_milletstoup room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletstoup"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornstoup cornstoup", () => {
  assert.ok(world.rooms.rust_cornstoup, "rust_cornstoup room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornstoup"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatstoup oatstoup", () => {
  assert.ok(world.rooms.rust_oatstoup, "rust_oatstoup room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatstoup"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumtumbler durumtumbler", () => {
  assert.ok(world.rooms.rust_durumtumbler, "rust_durumtumbler room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumtumbler"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniotumbler foniotumbler", () => {
  assert.ok(world.rooms.rust_foniotumbler, "rust_foniotumbler room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniotumbler"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefftumbler tefftumbler", () => {
  assert.ok(world.rooms.rust_tefftumbler, "rust_tefftumbler room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefftumbler"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleytumbler barleytumbler", () => {
  assert.ok(world.rooms.rust_barleytumbler, "rust_barleytumbler room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleytumbler"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumtumbler sorghumtumbler", () => {
  assert.ok(world.rooms.rust_sorghumtumbler, "rust_sorghumtumbler room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumtumbler"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millettumbler millettumbler", () => {
  assert.ok(world.rooms.rust_millettumbler, "rust_millettumbler room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millettumbler"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corntumbler corntumbler", () => {
  assert.ok(world.rooms.rust_corntumbler, "rust_corntumbler room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corntumbler"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oattumbler oattumbler", () => {
  assert.ok(world.rooms.rust_oattumbler, "rust_oattumbler room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oattumbler"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbeaker durumbeaker", () => {
  assert.ok(world.rooms.rust_durumbeaker, "rust_durumbeaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbeaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobeaker foniobeaker", () => {
  assert.ok(world.rooms.rust_foniobeaker, "rust_foniobeaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobeaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbeaker teffbeaker", () => {
  assert.ok(world.rooms.rust_teffbeaker, "rust_teffbeaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbeaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybeaker barleybeaker", () => {
  assert.ok(world.rooms.rust_barleybeaker, "rust_barleybeaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybeaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbeaker sorghumbeaker", () => {
  assert.ok(world.rooms.rust_sorghumbeaker, "rust_sorghumbeaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbeaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbeaker milletbeaker", () => {
  assert.ok(world.rooms.rust_milletbeaker, "rust_milletbeaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbeaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbeaker cornbeaker", () => {
  assert.ok(world.rooms.rust_cornbeaker, "rust_cornbeaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbeaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbeaker oatbeaker", () => {
  assert.ok(world.rooms.rust_oatbeaker, "rust_oatbeaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbeaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumstein durumstein", () => {
  assert.ok(world.rooms.rust_durumstein, "rust_durumstein room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumstein"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniostein foniostein", () => {
  assert.ok(world.rooms.rust_foniostein, "rust_foniostein room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniostein"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffstein teffstein", () => {
  assert.ok(world.rooms.rust_teffstein, "rust_teffstein room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffstein"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleystein barleystein", () => {
  assert.ok(world.rooms.rust_barleystein, "rust_barleystein room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleystein"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumstein sorghumstein", () => {
  assert.ok(world.rooms.rust_sorghumstein, "rust_sorghumstein room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumstein"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletstein milletstein", () => {
  assert.ok(world.rooms.rust_milletstein, "rust_milletstein room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletstein"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornstein cornstein", () => {
  assert.ok(world.rooms.rust_cornstein, "rust_cornstein room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornstein"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatstein oatstein", () => {
  assert.ok(world.rooms.rust_oatstein, "rust_oatstein room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatstein"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durummug durummug", () => {
  assert.ok(world.rooms.rust_durummug, "rust_durummug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durummug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniomug foniomug", () => {
  assert.ok(world.rooms.rust_foniomug, "rust_foniomug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniomug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffmug teffmug", () => {
  assert.ok(world.rooms.rust_teffmug, "rust_teffmug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffmug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleymug barleymug", () => {
  assert.ok(world.rooms.rust_barleymug, "rust_barleymug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleymug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghummug sorghummug", () => {
  assert.ok(world.rooms.rust_sorghummug, "rust_sorghummug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghummug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletmug milletmug", () => {
  assert.ok(world.rooms.rust_milletmug, "rust_milletmug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletmug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornmug cornmug", () => {
  assert.ok(world.rooms.rust_cornmug, "rust_cornmug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornmug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatmug oatmug", () => {
  assert.ok(world.rooms.rust_oatmug, "rust_oatmug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatmug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumgoblet durumgoblet", () => {
  assert.ok(world.rooms.rust_durumgoblet, "rust_durumgoblet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumgoblet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniogoblet foniogoblet", () => {
  assert.ok(world.rooms.rust_foniogoblet, "rust_foniogoblet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniogoblet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffgoblet teffgoblet", () => {
  assert.ok(world.rooms.rust_teffgoblet, "rust_teffgoblet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffgoblet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleygoblet barleygoblet", () => {
  assert.ok(world.rooms.rust_barleygoblet, "rust_barleygoblet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleygoblet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumgoblet sorghumgoblet", () => {
  assert.ok(world.rooms.rust_sorghumgoblet, "rust_sorghumgoblet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumgoblet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletgoblet milletgoblet", () => {
  assert.ok(world.rooms.rust_milletgoblet, "rust_milletgoblet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletgoblet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corngoblet corngoblet", () => {
  assert.ok(world.rooms.rust_corngoblet, "rust_corngoblet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corngoblet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatgoblet oatgoblet", () => {
  assert.ok(world.rooms.rust_oatgoblet, "rust_oatgoblet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatgoblet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumtankard durumtankard", () => {
  assert.ok(world.rooms.rust_durumtankard, "rust_durumtankard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumtankard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniotankard foniotankard", () => {
  assert.ok(world.rooms.rust_foniotankard, "rust_foniotankard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniotankard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefftankard tefftankard", () => {
  assert.ok(world.rooms.rust_tefftankard, "rust_tefftankard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefftankard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleytankard barleytankard", () => {
  assert.ok(world.rooms.rust_barleytankard, "rust_barleytankard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleytankard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumtankard sorghumtankard", () => {
  assert.ok(world.rooms.rust_sorghumtankard, "rust_sorghumtankard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumtankard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millettankard millettankard", () => {
  assert.ok(world.rooms.rust_millettankard, "rust_millettankard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millettankard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corntankard corntankard", () => {
  assert.ok(world.rooms.rust_corntankard, "rust_corntankard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corntankard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oattankard oattankard", () => {
  assert.ok(world.rooms.rust_oattankard, "rust_oattankard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oattankard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumcistern durumcistern", () => {
  assert.ok(world.rooms.rust_durumcistern, "rust_durumcistern room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumcistern"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniocistern foniocistern", () => {
  assert.ok(world.rooms.rust_foniocistern, "rust_foniocistern room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniocistern"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffcistern teffcistern", () => {
  assert.ok(world.rooms.rust_teffcistern, "rust_teffcistern room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffcistern"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleycistern barleycistern", () => {
  assert.ok(world.rooms.rust_barleycistern, "rust_barleycistern room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleycistern"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumcistern sorghumcistern", () => {
  assert.ok(world.rooms.rust_sorghumcistern, "rust_sorghumcistern room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumcistern"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletcistern milletcistern", () => {
  assert.ok(world.rooms.rust_milletcistern, "rust_milletcistern room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletcistern"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corncistern corncistern", () => {
  assert.ok(world.rooms.rust_corncistern, "rust_corncistern room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corncistern"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatcistern oatcistern", () => {
  assert.ok(world.rooms.rust_oatcistern, "rust_oatcistern room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatcistern"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumpipe durumpipe", () => {
  assert.ok(world.rooms.rust_durumpipe, "rust_durumpipe room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumpipe"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniopipe foniopipe", () => {
  assert.ok(world.rooms.rust_foniopipe, "rust_foniopipe room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniopipe"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffpipe teffpipe", () => {
  assert.ok(world.rooms.rust_teffpipe, "rust_teffpipe room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffpipe"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleypipe barleypipe", () => {
  assert.ok(world.rooms.rust_barleypipe, "rust_barleypipe room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleypipe"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumpipe sorghumpipe", () => {
  assert.ok(world.rooms.rust_sorghumpipe, "rust_sorghumpipe room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumpipe"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletpipe milletpipe", () => {
  assert.ok(world.rooms.rust_milletpipe, "rust_milletpipe room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletpipe"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornpipe cornpipe", () => {
  assert.ok(world.rooms.rust_cornpipe, "rust_cornpipe room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornpipe"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatpipe oatpipe", () => {
  assert.ok(world.rooms.rust_oatpipe, "rust_oatpipe room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatpipe"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbutt durumbutt", () => {
  assert.ok(world.rooms.rust_durumbutt, "rust_durumbutt room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbutt"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobutt foniobutt", () => {
  assert.ok(world.rooms.rust_foniobutt, "rust_foniobutt room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobutt"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbutt teffbutt", () => {
  assert.ok(world.rooms.rust_teffbutt, "rust_teffbutt room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbutt"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybutt barleybutt", () => {
  assert.ok(world.rooms.rust_barleybutt, "rust_barleybutt room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybutt"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbutt sorghumbutt", () => {
  assert.ok(world.rooms.rust_sorghumbutt, "rust_sorghumbutt room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbutt"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbutt milletbutt", () => {
  assert.ok(world.rooms.rust_milletbutt, "rust_milletbutt room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbutt"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbutt cornbutt", () => {
  assert.ok(world.rooms.rust_cornbutt, "rust_cornbutt room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbutt"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbutt oatbutt", () => {
  assert.ok(world.rooms.rust_oatbutt, "rust_oatbutt room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbutt"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumpuncheon durumpuncheon", () => {
  assert.ok(world.rooms.rust_durumpuncheon, "rust_durumpuncheon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumpuncheon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniopuncheon foniopuncheon", () => {
  assert.ok(world.rooms.rust_foniopuncheon, "rust_foniopuncheon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniopuncheon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffpuncheon teffpuncheon", () => {
  assert.ok(world.rooms.rust_teffpuncheon, "rust_teffpuncheon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffpuncheon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleypuncheon barleypuncheon", () => {
  assert.ok(world.rooms.rust_barleypuncheon, "rust_barleypuncheon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleypuncheon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumpuncheon sorghumpuncheon", () => {
  assert.ok(world.rooms.rust_sorghumpuncheon, "rust_sorghumpuncheon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumpuncheon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletpuncheon milletpuncheon", () => {
  assert.ok(world.rooms.rust_milletpuncheon, "rust_milletpuncheon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletpuncheon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornpuncheon cornpuncheon", () => {
  assert.ok(world.rooms.rust_cornpuncheon, "rust_cornpuncheon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornpuncheon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatpuncheon oatpuncheon", () => {
  assert.ok(world.rooms.rust_oatpuncheon, "rust_oatpuncheon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatpuncheon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumhogshead durumhogshead", () => {
  assert.ok(world.rooms.rust_durumhogshead, "rust_durumhogshead room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumhogshead"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniohogshead foniohogshead", () => {
  assert.ok(world.rooms.rust_foniohogshead, "rust_foniohogshead room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniohogshead"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffhogshead teffhogshead", () => {
  assert.ok(world.rooms.rust_teffhogshead, "rust_teffhogshead room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffhogshead"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyhogshead barleyhogshead", () => {
  assert.ok(world.rooms.rust_barleyhogshead, "rust_barleyhogshead room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyhogshead"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumhogshead sorghumhogshead", () => {
  assert.ok(world.rooms.rust_sorghumhogshead, "rust_sorghumhogshead room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumhogshead"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millethogshead millethogshead", () => {
  assert.ok(world.rooms.rust_millethogshead, "rust_millethogshead room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millethogshead"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornhogshead cornhogshead", () => {
  assert.ok(world.rooms.rust_cornhogshead, "rust_cornhogshead room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornhogshead"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oathogshead oathogshead", () => {
  assert.ok(world.rooms.rust_oathogshead, "rust_oathogshead room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oathogshead"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumtierce durumtierce", () => {
  assert.ok(world.rooms.rust_durumtierce, "rust_durumtierce room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumtierce"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniotierce foniotierce", () => {
  assert.ok(world.rooms.rust_foniotierce, "rust_foniotierce room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniotierce"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefftierce tefftierce", () => {
  assert.ok(world.rooms.rust_tefftierce, "rust_tefftierce room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefftierce"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleytierce barleytierce", () => {
  assert.ok(world.rooms.rust_barleytierce, "rust_barleytierce room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleytierce"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumtierce sorghumtierce", () => {
  assert.ok(world.rooms.rust_sorghumtierce, "rust_sorghumtierce room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumtierce"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millettierce millettierce", () => {
  assert.ok(world.rooms.rust_millettierce, "rust_millettierce room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millettierce"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corntierce corntierce", () => {
  assert.ok(world.rooms.rust_corntierce, "rust_corntierce room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corntierce"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oattierce oattierce", () => {
  assert.ok(world.rooms.rust_oattierce, "rust_oattierce room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oattierce"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumrundlet durumrundlet", () => {
  assert.ok(world.rooms.rust_durumrundlet, "rust_durumrundlet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumrundlet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniorundlet foniorundlet", () => {
  assert.ok(world.rooms.rust_foniorundlet, "rust_foniorundlet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniorundlet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffrundlet teffrundlet", () => {
  assert.ok(world.rooms.rust_teffrundlet, "rust_teffrundlet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffrundlet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyrundlet barleyrundlet", () => {
  assert.ok(world.rooms.rust_barleyrundlet, "rust_barleyrundlet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyrundlet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumrundlet sorghumrundlet", () => {
  assert.ok(world.rooms.rust_sorghumrundlet, "rust_sorghumrundlet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumrundlet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletrundlet milletrundlet", () => {
  assert.ok(world.rooms.rust_milletrundlet, "rust_milletrundlet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletrundlet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornrundlet cornrundlet", () => {
  assert.ok(world.rooms.rust_cornrundlet, "rust_cornrundlet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornrundlet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatrundlet oatrundlet", () => {
  assert.ok(world.rooms.rust_oatrundlet, "rust_oatrundlet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatrundlet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumkilderkin durumkilderkin", () => {
  assert.ok(world.rooms.rust_durumkilderkin, "rust_durumkilderkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumkilderkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniokilderkin foniokilderkin", () => {
  assert.ok(world.rooms.rust_foniokilderkin, "rust_foniokilderkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniokilderkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffkilderkin teffkilderkin", () => {
  assert.ok(world.rooms.rust_teffkilderkin, "rust_teffkilderkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffkilderkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleykilderkin barleykilderkin", () => {
  assert.ok(world.rooms.rust_barleykilderkin, "rust_barleykilderkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleykilderkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumkilderkin sorghumkilderkin", () => {
  assert.ok(world.rooms.rust_sorghumkilderkin, "rust_sorghumkilderkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumkilderkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletkilderkin milletkilderkin", () => {
  assert.ok(world.rooms.rust_milletkilderkin, "rust_milletkilderkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletkilderkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornkilderkin cornkilderkin", () => {
  assert.ok(world.rooms.rust_cornkilderkin, "rust_cornkilderkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornkilderkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatkilderkin oatkilderkin", () => {
  assert.ok(world.rooms.rust_oatkilderkin, "rust_oatkilderkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatkilderkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumfirkin durumfirkin", () => {
  assert.ok(world.rooms.rust_durumfirkin, "rust_durumfirkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumfirkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniofirkin foniofirkin", () => {
  assert.ok(world.rooms.rust_foniofirkin, "rust_foniofirkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniofirkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefffirkin tefffirkin", () => {
  assert.ok(world.rooms.rust_tefffirkin, "rust_tefffirkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefffirkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyfirkin barleyfirkin", () => {
  assert.ok(world.rooms.rust_barleyfirkin, "rust_barleyfirkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyfirkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumfirkin sorghumfirkin", () => {
  assert.ok(world.rooms.rust_sorghumfirkin, "rust_sorghumfirkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumfirkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletfirkin milletfirkin", () => {
  assert.ok(world.rooms.rust_milletfirkin, "rust_milletfirkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletfirkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornfirkin cornfirkin", () => {
  assert.ok(world.rooms.rust_cornfirkin, "rust_cornfirkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornfirkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatfirkin oatfirkin", () => {
  assert.ok(world.rooms.rust_oatfirkin, "rust_oatfirkin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatfirkin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbarrel durumbarrel", () => {
  assert.ok(world.rooms.rust_durumbarrel, "rust_durumbarrel room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbarrel"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobarrel foniobarrel", () => {
  assert.ok(world.rooms.rust_foniobarrel, "rust_foniobarrel room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobarrel"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbarrel teffbarrel", () => {
  assert.ok(world.rooms.rust_teffbarrel, "rust_teffbarrel room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbarrel"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybarrel barleybarrel", () => {
  assert.ok(world.rooms.rust_barleybarrel, "rust_barleybarrel room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybarrel"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbarrel sorghumbarrel", () => {
  assert.ok(world.rooms.rust_sorghumbarrel, "rust_sorghumbarrel room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbarrel"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbarrel milletbarrel", () => {
  assert.ok(world.rooms.rust_milletbarrel, "rust_milletbarrel room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbarrel"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbarrel cornbarrel", () => {
  assert.ok(world.rooms.rust_cornbarrel, "rust_cornbarrel room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbarrel"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbarrel oatbarrel", () => {
  assert.ok(world.rooms.rust_oatbarrel, "rust_oatbarrel room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbarrel"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumtun durumtun", () => {
  assert.ok(world.rooms.rust_durumtun, "rust_durumtun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumtun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniotun foniotun", () => {
  assert.ok(world.rooms.rust_foniotun, "rust_foniotun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniotun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefftun tefftun", () => {
  assert.ok(world.rooms.rust_tefftun, "rust_tefftun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefftun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleytun barleytun", () => {
  assert.ok(world.rooms.rust_barleytun, "rust_barleytun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleytun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumtun sorghumtun", () => {
  assert.ok(world.rooms.rust_sorghumtun, "rust_sorghumtun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumtun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millettun millettun", () => {
  assert.ok(world.rooms.rust_millettun, "rust_millettun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millettun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corntun corntun", () => {
  assert.ok(world.rooms.rust_corntun, "rust_corntun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corntun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oattun oattun", () => {
  assert.ok(world.rooms.rust_oattun, "rust_oattun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oattun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumamphora durumamphora", () => {
  assert.ok(world.rooms.rust_durumamphora, "rust_durumamphora room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumamphora"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioamphora fonioamphora", () => {
  assert.ok(world.rooms.rust_fonioamphora, "rust_fonioamphora room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioamphora"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffamphora teffamphora", () => {
  assert.ok(world.rooms.rust_teffamphora, "rust_teffamphora room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffamphora"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyamphora barleyamphora", () => {
  assert.ok(world.rooms.rust_barleyamphora, "rust_barleyamphora room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyamphora"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumamphora sorghumamphora", () => {
  assert.ok(world.rooms.rust_sorghumamphora, "rust_sorghumamphora room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumamphora"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletamphora milletamphora", () => {
  assert.ok(world.rooms.rust_milletamphora, "rust_milletamphora room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletamphora"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornamphora cornamphora", () => {
  assert.ok(world.rooms.rust_cornamphora, "rust_cornamphora room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornamphora"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatamphora oatamphora", () => {
  assert.ok(world.rooms.rust_oatamphora, "rust_oatamphora room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatamphora"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumdemijohn durumdemijohn", () => {
  assert.ok(world.rooms.rust_durumdemijohn, "rust_durumdemijohn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumdemijohn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniodemijohn foniodemijohn", () => {
  assert.ok(world.rooms.rust_foniodemijohn, "rust_foniodemijohn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniodemijohn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffdemijohn teffdemijohn", () => {
  assert.ok(world.rooms.rust_teffdemijohn, "rust_teffdemijohn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffdemijohn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleydemijohn barleydemijohn", () => {
  assert.ok(world.rooms.rust_barleydemijohn, "rust_barleydemijohn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleydemijohn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumdemijohn sorghumdemijohn", () => {
  assert.ok(world.rooms.rust_sorghumdemijohn, "rust_sorghumdemijohn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumdemijohn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletdemijohn milletdemijohn", () => {
  assert.ok(world.rooms.rust_milletdemijohn, "rust_milletdemijohn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletdemijohn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corndemijohn corndemijohn", () => {
  assert.ok(world.rooms.rust_corndemijohn, "rust_corndemijohn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corndemijohn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatdemijohn oatdemijohn", () => {
  assert.ok(world.rooms.rust_oatdemijohn, "rust_oatdemijohn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatdemijohn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumcarboy durumcarboy", () => {
  assert.ok(world.rooms.rust_durumcarboy, "rust_durumcarboy room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumcarboy"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniocarboy foniocarboy", () => {
  assert.ok(world.rooms.rust_foniocarboy, "rust_foniocarboy room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniocarboy"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffcarboy teffcarboy", () => {
  assert.ok(world.rooms.rust_teffcarboy, "rust_teffcarboy room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffcarboy"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleycarboy barleycarboy", () => {
  assert.ok(world.rooms.rust_barleycarboy, "rust_barleycarboy room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleycarboy"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumcarboy sorghumcarboy", () => {
  assert.ok(world.rooms.rust_sorghumcarboy, "rust_sorghumcarboy room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumcarboy"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletcarboy milletcarboy", () => {
  assert.ok(world.rooms.rust_milletcarboy, "rust_milletcarboy room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletcarboy"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corncarboy corncarboy", () => {
  assert.ok(world.rooms.rust_corncarboy, "rust_corncarboy room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corncarboy"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatcarboy oatcarboy", () => {
  assert.ok(world.rooms.rust_oatcarboy, "rust_oatcarboy room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatcarboy"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumampule durumampule", () => {
  assert.ok(world.rooms.rust_durumampule, "rust_durumampule room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumampule"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioampule fonioampule", () => {
  assert.ok(world.rooms.rust_fonioampule, "rust_fonioampule room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioampule"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffampule teffampule", () => {
  assert.ok(world.rooms.rust_teffampule, "rust_teffampule room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffampule"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyampule barleyampule", () => {
  assert.ok(world.rooms.rust_barleyampule, "rust_barleyampule room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyampule"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumampule sorghumampule", () => {
  assert.ok(world.rooms.rust_sorghumampule, "rust_sorghumampule room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumampule"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletampule milletampule", () => {
  assert.ok(world.rooms.rust_milletampule, "rust_milletampule room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletampule"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornampule cornampule", () => {
  assert.ok(world.rooms.rust_cornampule, "rust_cornampule room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornampule"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatampule oatampule", () => {
  assert.ok(world.rooms.rust_oatampule, "rust_oatampule room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatampule"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumphial durumphial", () => {
  assert.ok(world.rooms.rust_durumphial, "rust_durumphial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumphial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniophial foniophial", () => {
  assert.ok(world.rooms.rust_foniophial, "rust_foniophial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniophial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffphial teffphial", () => {
  assert.ok(world.rooms.rust_teffphial, "rust_teffphial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffphial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyphial barleyphial", () => {
  assert.ok(world.rooms.rust_barleyphial, "rust_barleyphial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyphial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumphial sorghumphial", () => {
  assert.ok(world.rooms.rust_sorghumphial, "rust_sorghumphial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumphial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletphial milletphial", () => {
  assert.ok(world.rooms.rust_milletphial, "rust_milletphial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletphial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornphial cornphial", () => {
  assert.ok(world.rooms.rust_cornphial, "rust_cornphial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornphial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatphial oatphial", () => {
  assert.ok(world.rooms.rust_oatphial, "rust_oatphial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatphial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumflagon durumflagon", () => {
  assert.ok(world.rooms.rust_durumflagon, "rust_durumflagon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumflagon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioflagon fonioflagon", () => {
  assert.ok(world.rooms.rust_fonioflagon, "rust_fonioflagon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioflagon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffflagon teffflagon", () => {
  assert.ok(world.rooms.rust_teffflagon, "rust_teffflagon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffflagon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyflagon barleyflagon", () => {
  assert.ok(world.rooms.rust_barleyflagon, "rust_barleyflagon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyflagon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumflagon sorghumflagon", () => {
  assert.ok(world.rooms.rust_sorghumflagon, "rust_sorghumflagon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumflagon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletflagon milletflagon", () => {
  assert.ok(world.rooms.rust_milletflagon, "rust_milletflagon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletflagon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornflagon cornflagon", () => {
  assert.ok(world.rooms.rust_cornflagon, "rust_cornflagon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornflagon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatflagon oatflagon", () => {
  assert.ok(world.rooms.rust_oatflagon, "rust_oatflagon room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatflagon"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumewer durumewer", () => {
  assert.ok(world.rooms.rust_durumewer, "rust_durumewer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumewer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioewer fonioewer", () => {
  assert.ok(world.rooms.rust_fonioewer, "rust_fonioewer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioewer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffewer teffewer", () => {
  assert.ok(world.rooms.rust_teffewer, "rust_teffewer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffewer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyewer barleyewer", () => {
  assert.ok(world.rooms.rust_barleyewer, "rust_barleyewer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyewer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumewer sorghumewer", () => {
  assert.ok(world.rooms.rust_sorghumewer, "rust_sorghumewer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumewer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletewer milletewer", () => {
  assert.ok(world.rooms.rust_milletewer, "rust_milletewer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletewer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornewer cornewer", () => {
  assert.ok(world.rooms.rust_cornewer, "rust_cornewer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornewer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatewer oatewer", () => {
  assert.ok(world.rooms.rust_oatewer, "rust_oatewer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatewer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumcruet durumcruet", () => {
  assert.ok(world.rooms.rust_durumcruet, "rust_durumcruet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumcruet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniocruet foniocruet", () => {
  assert.ok(world.rooms.rust_foniocruet, "rust_foniocruet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniocruet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffcruet teffcruet", () => {
  assert.ok(world.rooms.rust_teffcruet, "rust_teffcruet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffcruet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleycruet barleycruet", () => {
  assert.ok(world.rooms.rust_barleycruet, "rust_barleycruet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleycruet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumcruet sorghumcruet", () => {
  assert.ok(world.rooms.rust_sorghumcruet, "rust_sorghumcruet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumcruet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletcruet milletcruet", () => {
  assert.ok(world.rooms.rust_milletcruet, "rust_milletcruet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletcruet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corncruet corncruet", () => {
  assert.ok(world.rooms.rust_corncruet, "rust_corncruet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corncruet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatcruet oatcruet", () => {
  assert.ok(world.rooms.rust_oatcruet, "rust_oatcruet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatcruet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumjug durumjug", () => {
  assert.ok(world.rooms.rust_durumjug, "rust_durumjug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumjug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniojug foniojug", () => {
  assert.ok(world.rooms.rust_foniojug, "rust_foniojug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniojug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffjug teffjug", () => {
  assert.ok(world.rooms.rust_teffjug, "rust_teffjug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffjug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyjug barleyjug", () => {
  assert.ok(world.rooms.rust_barleyjug, "rust_barleyjug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyjug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumjug sorghumjug", () => {
  assert.ok(world.rooms.rust_sorghumjug, "rust_sorghumjug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumjug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletjug milletjug", () => {
  assert.ok(world.rooms.rust_milletjug, "rust_milletjug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletjug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornjug cornjug", () => {
  assert.ok(world.rooms.rust_cornjug, "rust_cornjug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornjug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatjug oatjug", () => {
  assert.ok(world.rooms.rust_oatjug, "rust_oatjug room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatjug"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbottle durumbottle", () => {
  assert.ok(world.rooms.rust_durumbottle, "rust_durumbottle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbottle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobottle foniobottle", () => {
  assert.ok(world.rooms.rust_foniobottle, "rust_foniobottle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobottle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbottle teffbottle", () => {
  assert.ok(world.rooms.rust_teffbottle, "rust_teffbottle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbottle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybottle barleybottle", () => {
  assert.ok(world.rooms.rust_barleybottle, "rust_barleybottle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybottle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbottle sorghumbottle", () => {
  assert.ok(world.rooms.rust_sorghumbottle, "rust_sorghumbottle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbottle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbottle milletbottle", () => {
  assert.ok(world.rooms.rust_milletbottle, "rust_milletbottle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbottle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbottle cornbottle", () => {
  assert.ok(world.rooms.rust_cornbottle, "rust_cornbottle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbottle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbottle oatbottle", () => {
  assert.ok(world.rooms.rust_oatbottle, "rust_oatbottle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbottle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumvial durumvial", () => {
  assert.ok(world.rooms.rust_durumvial, "rust_durumvial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumvial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniovial foniovial", () => {
  assert.ok(world.rooms.rust_foniovial, "rust_foniovial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniovial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffvial teffvial", () => {
  assert.ok(world.rooms.rust_teffvial, "rust_teffvial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffvial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyvial barleyvial", () => {
  assert.ok(world.rooms.rust_barleyvial, "rust_barleyvial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyvial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumvial sorghumvial", () => {
  assert.ok(world.rooms.rust_sorghumvial, "rust_sorghumvial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumvial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletvial milletvial", () => {
  assert.ok(world.rooms.rust_milletvial, "rust_milletvial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletvial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornvial cornvial", () => {
  assert.ok(world.rooms.rust_cornvial, "rust_cornvial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornvial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatvial oatvial", () => {
  assert.ok(world.rooms.rust_oatvial, "rust_oatvial room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatvial"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumflask durumflask", () => {
  assert.ok(world.rooms.rust_durumflask, "rust_durumflask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumflask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioflask fonioflask", () => {
  assert.ok(world.rooms.rust_fonioflask, "rust_fonioflask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioflask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffflask teffflask", () => {
  assert.ok(world.rooms.rust_teffflask, "rust_teffflask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffflask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyflask barleyflask", () => {
  assert.ok(world.rooms.rust_barleyflask, "rust_barleyflask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyflask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumflask sorghumflask", () => {
  assert.ok(world.rooms.rust_sorghumflask, "rust_sorghumflask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumflask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletflask milletflask", () => {
  assert.ok(world.rooms.rust_milletflask, "rust_milletflask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletflask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornflask cornflask", () => {
  assert.ok(world.rooms.rust_cornflask, "rust_cornflask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornflask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatflask oatflask", () => {
  assert.ok(world.rooms.rust_oatflask, "rust_oatflask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatflask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumurn durumurn", () => {
  assert.ok(world.rooms.rust_durumurn, "rust_durumurn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumurn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniourn foniourn", () => {
  assert.ok(world.rooms.rust_foniourn, "rust_foniourn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniourn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffurn teffurn", () => {
  assert.ok(world.rooms.rust_teffurn, "rust_teffurn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffurn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyurn barleyurn", () => {
  assert.ok(world.rooms.rust_barleyurn, "rust_barleyurn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyurn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumurn sorghumurn", () => {
  assert.ok(world.rooms.rust_sorghumurn, "rust_sorghumurn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumurn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milleturn milleturn", () => {
  assert.ok(world.rooms.rust_milleturn, "rust_milleturn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milleturn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornurn cornurn", () => {
  assert.ok(world.rooms.rust_cornurn, "rust_cornurn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornurn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oaturn oaturn", () => {
  assert.ok(world.rooms.rust_oaturn, "rust_oaturn room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oaturn"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumscoop durumscoop", () => {
  assert.ok(world.rooms.rust_durumscoop, "rust_durumscoop room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumscoop"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioscoop fonioscoop", () => {
  assert.ok(world.rooms.rust_fonioscoop, "rust_fonioscoop room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioscoop"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffscoop teffscoop", () => {
  assert.ok(world.rooms.rust_teffscoop, "rust_teffscoop room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffscoop"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyscoop barleyscoop", () => {
  assert.ok(world.rooms.rust_barleyscoop, "rust_barleyscoop room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyscoop"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumscoop sorghumscoop", () => {
  assert.ok(world.rooms.rust_sorghumscoop, "rust_sorghumscoop room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumscoop"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletscoop milletscoop", () => {
  assert.ok(world.rooms.rust_milletscoop, "rust_milletscoop room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletscoop"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornscoop cornscoop", () => {
  assert.ok(world.rooms.rust_cornscoop, "rust_cornscoop room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornscoop"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatscoop oatscoop", () => {
  assert.ok(world.rooms.rust_oatscoop, "rust_oatscoop room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatscoop"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumladle durumladle", () => {
  assert.ok(world.rooms.rust_durumladle, "rust_durumladle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumladle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioladle fonioladle", () => {
  assert.ok(world.rooms.rust_fonioladle, "rust_fonioladle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioladle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffladle teffladle", () => {
  assert.ok(world.rooms.rust_teffladle, "rust_teffladle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffladle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyladle barleyladle", () => {
  assert.ok(world.rooms.rust_barleyladle, "rust_barleyladle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyladle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumladle sorghumladle", () => {
  assert.ok(world.rooms.rust_sorghumladle, "rust_sorghumladle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumladle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletladle milletladle", () => {
  assert.ok(world.rooms.rust_milletladle, "rust_milletladle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletladle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornladle cornladle", () => {
  assert.ok(world.rooms.rust_cornladle, "rust_cornladle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornladle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatladle oatladle", () => {
  assert.ok(world.rooms.rust_oatladle, "rust_oatladle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatladle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumtray durumtray", () => {
  assert.ok(world.rooms.rust_durumtray, "rust_durumtray room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumtray"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniotray foniotray", () => {
  assert.ok(world.rooms.rust_foniotray, "rust_foniotray room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniotray"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefftray tefftray", () => {
  assert.ok(world.rooms.rust_tefftray, "rust_tefftray room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefftray"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleytray barleytray", () => {
  assert.ok(world.rooms.rust_barleytray, "rust_barleytray room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleytray"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumtray sorghumtray", () => {
  assert.ok(world.rooms.rust_sorghumtray, "rust_sorghumtray room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumtray"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millettray millettray", () => {
  assert.ok(world.rooms.rust_millettray, "rust_millettray room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millettray"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corntray corntray", () => {
  assert.ok(world.rooms.rust_corntray, "rust_corntray room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corntray"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oattray oattray", () => {
  assert.ok(world.rooms.rust_oattray, "rust_oattray room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oattray"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbasin durumbasin", () => {
  assert.ok(world.rooms.rust_durumbasin, "rust_durumbasin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbasin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobasin foniobasin", () => {
  assert.ok(world.rooms.rust_foniobasin, "rust_foniobasin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobasin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbasin teffbasin", () => {
  assert.ok(world.rooms.rust_teffbasin, "rust_teffbasin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbasin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybasin barleybasin", () => {
  assert.ok(world.rooms.rust_barleybasin, "rust_barleybasin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybasin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbasin sorghumbasin", () => {
  assert.ok(world.rooms.rust_sorghumbasin, "rust_sorghumbasin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbasin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbasin milletbasin", () => {
  assert.ok(world.rooms.rust_milletbasin, "rust_milletbasin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbasin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbasin cornbasin", () => {
  assert.ok(world.rooms.rust_cornbasin, "rust_cornbasin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbasin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbasin oatbasin", () => {
  assert.ok(world.rooms.rust_oatbasin, "rust_oatbasin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbasin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumcrock durumcrock", () => {
  assert.ok(world.rooms.rust_durumcrock, "rust_durumcrock room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumcrock"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniocrock foniocrock", () => {
  assert.ok(world.rooms.rust_foniocrock, "rust_foniocrock room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniocrock"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffcrock teffcrock", () => {
  assert.ok(world.rooms.rust_teffcrock, "rust_teffcrock room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffcrock"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleycrock barleycrock", () => {
  assert.ok(world.rooms.rust_barleycrock, "rust_barleycrock room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleycrock"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumcrock sorghumcrock", () => {
  assert.ok(world.rooms.rust_sorghumcrock, "rust_sorghumcrock room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumcrock"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletcrock milletcrock", () => {
  assert.ok(world.rooms.rust_milletcrock, "rust_milletcrock room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletcrock"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corncrock corncrock", () => {
  assert.ok(world.rooms.rust_corncrock, "rust_corncrock room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corncrock"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatcrock oatcrock", () => {
  assert.ok(world.rooms.rust_oatcrock, "rust_oatcrock room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatcrock"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumskillet durumskillet", () => {
  assert.ok(world.rooms.rust_durumskillet, "rust_durumskillet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumskillet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioskillet fonioskillet", () => {
  assert.ok(world.rooms.rust_fonioskillet, "rust_fonioskillet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioskillet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffskillet teffskillet", () => {
  assert.ok(world.rooms.rust_teffskillet, "rust_teffskillet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffskillet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyskillet barleyskillet", () => {
  assert.ok(world.rooms.rust_barleyskillet, "rust_barleyskillet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyskillet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumskillet sorghumskillet", () => {
  assert.ok(world.rooms.rust_sorghumskillet, "rust_sorghumskillet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumskillet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletskillet milletskillet", () => {
  assert.ok(world.rooms.rust_milletskillet, "rust_milletskillet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletskillet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornskillet cornskillet", () => {
  assert.ok(world.rooms.rust_cornskillet, "rust_cornskillet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornskillet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatskillet oatskillet", () => {
  assert.ok(world.rooms.rust_oatskillet, "rust_oatskillet room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatskillet"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumkettle durumkettle", () => {
  assert.ok(world.rooms.rust_durumkettle, "rust_durumkettle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumkettle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniokettle foniokettle", () => {
  assert.ok(world.rooms.rust_foniokettle, "rust_foniokettle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniokettle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffkettle teffkettle", () => {
  assert.ok(world.rooms.rust_teffkettle, "rust_teffkettle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffkettle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleykettle barleykettle", () => {
  assert.ok(world.rooms.rust_barleykettle, "rust_barleykettle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleykettle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumkettle sorghumkettle", () => {
  assert.ok(world.rooms.rust_sorghumkettle, "rust_sorghumkettle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumkettle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletkettle milletkettle", () => {
  assert.ok(world.rooms.rust_milletkettle, "rust_milletkettle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletkettle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornkettle cornkettle", () => {
  assert.ok(world.rooms.rust_cornkettle, "rust_cornkettle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornkettle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatkettle oatkettle", () => {
  assert.ok(world.rooms.rust_oatkettle, "rust_oatkettle room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatkettle"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbucket durumbucket", () => {
  assert.ok(world.rooms.rust_durumbucket, "rust_durumbucket room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbucket"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobucket foniobucket", () => {
  assert.ok(world.rooms.rust_foniobucket, "rust_foniobucket room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobucket"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbucket teffbucket", () => {
  assert.ok(world.rooms.rust_teffbucket, "rust_teffbucket room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbucket"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybucket barleybucket", () => {
  assert.ok(world.rooms.rust_barleybucket, "rust_barleybucket room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybucket"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbucket sorghumbucket", () => {
  assert.ok(world.rooms.rust_sorghumbucket, "rust_sorghumbucket room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbucket"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbucket milletbucket", () => {
  assert.ok(world.rooms.rust_milletbucket, "rust_milletbucket room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbucket"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbucket cornbucket", () => {
  assert.ok(world.rooms.rust_cornbucket, "rust_cornbucket room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbucket"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbucket oatbucket", () => {
  assert.ok(world.rooms.rust_oatbucket, "rust_oatbucket room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbucket"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbowl durumbowl", () => {
  assert.ok(world.rooms.rust_durumbowl, "rust_durumbowl room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbowl"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobowl foniobowl", () => {
  assert.ok(world.rooms.rust_foniobowl, "rust_foniobowl room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobowl"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbowl teffbowl", () => {
  assert.ok(world.rooms.rust_teffbowl, "rust_teffbowl room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbowl"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybowl barleybowl", () => {
  assert.ok(world.rooms.rust_barleybowl, "rust_barleybowl room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybowl"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbowl sorghumbowl", () => {
  assert.ok(world.rooms.rust_sorghumbowl, "rust_sorghumbowl room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbowl"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbowl milletbowl", () => {
  assert.ok(world.rooms.rust_milletbowl, "rust_milletbowl room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbowl"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbowl cornbowl", () => {
  assert.ok(world.rooms.rust_cornbowl, "rust_cornbowl room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbowl"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbowl oatbowl", () => {
  assert.ok(world.rooms.rust_oatbowl, "rust_oatbowl room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbowl"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumpot durumpot", () => {
  assert.ok(world.rooms.rust_durumpot, "rust_durumpot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumpot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniopot foniopot", () => {
  assert.ok(world.rooms.rust_foniopot, "rust_foniopot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniopot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffpot teffpot", () => {
  assert.ok(world.rooms.rust_teffpot, "rust_teffpot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffpot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleypot barleypot", () => {
  assert.ok(world.rooms.rust_barleypot, "rust_barleypot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleypot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumpot sorghumpot", () => {
  assert.ok(world.rooms.rust_sorghumpot, "rust_sorghumpot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumpot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletpot milletpot", () => {
  assert.ok(world.rooms.rust_milletpot, "rust_milletpot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletpot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornpot cornpot", () => {
  assert.ok(world.rooms.rust_cornpot, "rust_cornpot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornpot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatpot oatpot", () => {
  assert.ok(world.rooms.rust_oatpot, "rust_oatpot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatpot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumpan durumpan", () => {
  assert.ok(world.rooms.rust_durumpan, "rust_durumpan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumpan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniopan foniopan", () => {
  assert.ok(world.rooms.rust_foniopan, "rust_foniopan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniopan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffpan teffpan", () => {
  assert.ok(world.rooms.rust_teffpan, "rust_teffpan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffpan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleypan barleypan", () => {
  assert.ok(world.rooms.rust_barleypan, "rust_barleypan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleypan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumpan sorghumpan", () => {
  assert.ok(world.rooms.rust_sorghumpan, "rust_sorghumpan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumpan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletpan milletpan", () => {
  assert.ok(world.rooms.rust_milletpan, "rust_milletpan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletpan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornpan cornpan", () => {
  assert.ok(world.rooms.rust_cornpan, "rust_cornpan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornpan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatpan oatpan", () => {
  assert.ok(world.rooms.rust_oatpan, "rust_oatpan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatpan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumtank durumtank", () => {
  assert.ok(world.rooms.rust_durumtank, "rust_durumtank room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumtank"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniotank foniotank", () => {
  assert.ok(world.rooms.rust_foniotank, "rust_foniotank room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniotank"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefftank tefftank", () => {
  assert.ok(world.rooms.rust_tefftank, "rust_tefftank room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefftank"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleytank barleytank", () => {
  assert.ok(world.rooms.rust_barleytank, "rust_barleytank room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleytank"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumtank sorghumtank", () => {
  assert.ok(world.rooms.rust_sorghumtank, "rust_sorghumtank room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumtank"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millettank millettank", () => {
  assert.ok(world.rooms.rust_millettank, "rust_millettank room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millettank"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corntank corntank", () => {
  assert.ok(world.rooms.rust_corntank, "rust_corntank room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corntank"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oattank oattank", () => {
  assert.ok(world.rooms.rust_oattank, "rust_oattank room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oattank"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumark durumark", () => {
  assert.ok(world.rooms.rust_durumark, "rust_durumark room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumark"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioark fonioark", () => {
  assert.ok(world.rooms.rust_fonioark, "rust_fonioark room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioark"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffark teffark", () => {
  assert.ok(world.rooms.rust_teffark, "rust_teffark room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffark"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyark barleyark", () => {
  assert.ok(world.rooms.rust_barleyark, "rust_barleyark room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyark"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumark sorghumark", () => {
  assert.ok(world.rooms.rust_sorghumark, "rust_sorghumark room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumark"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletark milletark", () => {
  assert.ok(world.rooms.rust_milletark, "rust_milletark room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletark"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornark cornark", () => {
  assert.ok(world.rooms.rust_cornark, "rust_cornark room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornark"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatark oatark", () => {
  assert.ok(world.rooms.rust_oatark, "rust_oatark room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatark"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumtote durumtote", () => {
  assert.ok(world.rooms.rust_durumtote, "rust_durumtote room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumtote"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniotote foniotote", () => {
  assert.ok(world.rooms.rust_foniotote, "rust_foniotote room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniotote"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefftote tefftote", () => {
  assert.ok(world.rooms.rust_tefftote, "rust_tefftote room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefftote"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleytote barleytote", () => {
  assert.ok(world.rooms.rust_barleytote, "rust_barleytote room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleytote"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumtote sorghumtote", () => {
  assert.ok(world.rooms.rust_sorghumtote, "rust_sorghumtote room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumtote"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millettote millettote", () => {
  assert.ok(world.rooms.rust_millettote, "rust_millettote room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millettote"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corntote corntote", () => {
  assert.ok(world.rooms.rust_corntote, "rust_corntote room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corntote"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oattote oattote", () => {
  assert.ok(world.rooms.rust_oattote, "rust_oattote room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oattote"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumpress durumpress", () => {
  assert.ok(world.rooms.rust_durumpress, "rust_durumpress room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumpress"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniopress foniopress", () => {
  assert.ok(world.rooms.rust_foniopress, "rust_foniopress room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniopress"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffpress teffpress", () => {
  assert.ok(world.rooms.rust_teffpress, "rust_teffpress room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffpress"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleypress barleypress", () => {
  assert.ok(world.rooms.rust_barleypress, "rust_barleypress room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleypress"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumpress sorghumpress", () => {
  assert.ok(world.rooms.rust_sorghumpress, "rust_sorghumpress room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumpress"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletpress milletpress", () => {
  assert.ok(world.rooms.rust_milletpress, "rust_milletpress room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletpress"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornpress cornpress", () => {
  assert.ok(world.rooms.rust_cornpress, "rust_cornpress room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornpress"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatpress oatpress", () => {
  assert.ok(world.rooms.rust_oatpress, "rust_oatpress room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatpress"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumsilo durumsilo", () => {
  assert.ok(world.rooms.rust_durumsilo, "rust_durumsilo room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumsilo"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniosilo foniosilo", () => {
  assert.ok(world.rooms.rust_foniosilo, "rust_foniosilo room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniosilo"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffsilo teffsilo", () => {
  assert.ok(world.rooms.rust_teffsilo, "rust_teffsilo room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffsilo"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleysilo barleysilo", () => {
  assert.ok(world.rooms.rust_barleysilo, "rust_barleysilo room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleysilo"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumsilo sorghumsilo", () => {
  assert.ok(world.rooms.rust_sorghumsilo, "rust_sorghumsilo room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumsilo"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletsilo milletsilo", () => {
  assert.ok(world.rooms.rust_milletsilo, "rust_milletsilo room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletsilo"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornsilo cornsilo", () => {
  assert.ok(world.rooms.rust_cornsilo, "rust_cornsilo room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornsilo"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatsilo oatsilo", () => {
  assert.ok(world.rooms.rust_oatsilo, "rust_oatsilo room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatsilo"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumhopper durumhopper", () => {
  assert.ok(world.rooms.rust_durumhopper, "rust_durumhopper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumhopper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniohopper foniohopper", () => {
  assert.ok(world.rooms.rust_foniohopper, "rust_foniohopper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniohopper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffhopper teffhopper", () => {
  assert.ok(world.rooms.rust_teffhopper, "rust_teffhopper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffhopper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyhopper barleyhopper", () => {
  assert.ok(world.rooms.rust_barleyhopper, "rust_barleyhopper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyhopper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumhopper sorghumhopper", () => {
  assert.ok(world.rooms.rust_sorghumhopper, "rust_sorghumhopper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumhopper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millethopper millethopper", () => {
  assert.ok(world.rooms.rust_millethopper, "rust_millethopper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millethopper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornhopper cornhopper", () => {
  assert.ok(world.rooms.rust_cornhopper, "rust_cornhopper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornhopper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oathopper oathopper", () => {
  assert.ok(world.rooms.rust_oathopper, "rust_oathopper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oathopper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbunker durumbunker", () => {
  assert.ok(world.rooms.rust_durumbunker, "rust_durumbunker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbunker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobunker foniobunker", () => {
  assert.ok(world.rooms.rust_foniobunker, "rust_foniobunker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobunker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbunker teffbunker", () => {
  assert.ok(world.rooms.rust_teffbunker, "rust_teffbunker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbunker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybunker barleybunker", () => {
  assert.ok(world.rooms.rust_barleybunker, "rust_barleybunker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybunker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbunker sorghumbunker", () => {
  assert.ok(world.rooms.rust_sorghumbunker, "rust_sorghumbunker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbunker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbunker milletbunker", () => {
  assert.ok(world.rooms.rust_milletbunker, "rust_milletbunker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbunker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbunker cornbunker", () => {
  assert.ok(world.rooms.rust_cornbunker, "rust_cornbunker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbunker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbunker oatbunker", () => {
  assert.ok(world.rooms.rust_oatbunker, "rust_oatbunker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbunker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumcoffer durumcoffer", () => {
  assert.ok(world.rooms.rust_durumcoffer, "rust_durumcoffer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumcoffer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniocoffer foniocoffer", () => {
  assert.ok(world.rooms.rust_foniocoffer, "rust_foniocoffer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniocoffer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffcoffer teffcoffer", () => {
  assert.ok(world.rooms.rust_teffcoffer, "rust_teffcoffer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffcoffer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleycoffer barleycoffer", () => {
  assert.ok(world.rooms.rust_barleycoffer, "rust_barleycoffer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleycoffer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumcoffer sorghumcoffer", () => {
  assert.ok(world.rooms.rust_sorghumcoffer, "rust_sorghumcoffer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumcoffer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletcoffer milletcoffer", () => {
  assert.ok(world.rooms.rust_milletcoffer, "rust_milletcoffer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletcoffer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corncoffer corncoffer", () => {
  assert.ok(world.rooms.rust_corncoffer, "rust_corncoffer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corncoffer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatcoffer oatcoffer", () => {
  assert.ok(world.rooms.rust_oatcoffer, "rust_oatcoffer room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatcoffer"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumhamper durumhamper", () => {
  assert.ok(world.rooms.rust_durumhamper, "rust_durumhamper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumhamper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniohamper foniohamper", () => {
  assert.ok(world.rooms.rust_foniohamper, "rust_foniohamper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniohamper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffhamper teffhamper", () => {
  assert.ok(world.rooms.rust_teffhamper, "rust_teffhamper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffhamper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyhamper barleyhamper", () => {
  assert.ok(world.rooms.rust_barleyhamper, "rust_barleyhamper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyhamper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumhamper sorghumhamper", () => {
  assert.ok(world.rooms.rust_sorghumhamper, "rust_sorghumhamper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumhamper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millethamper millethamper", () => {
  assert.ok(world.rooms.rust_millethamper, "rust_millethamper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millethamper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornhamper cornhamper", () => {
  assert.ok(world.rooms.rust_cornhamper, "rust_cornhamper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornhamper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oathamper oathamper", () => {
  assert.ok(world.rooms.rust_oathamper, "rust_oathamper room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oathamper"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumtrunk durumtrunk", () => {
  assert.ok(world.rooms.rust_durumtrunk, "rust_durumtrunk room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumtrunk"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniotrunk foniotrunk", () => {
  assert.ok(world.rooms.rust_foniotrunk, "rust_foniotrunk room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniotrunk"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefftrunk tefftrunk", () => {
  assert.ok(world.rooms.rust_tefftrunk, "rust_tefftrunk room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefftrunk"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleytrunk barleytrunk", () => {
  assert.ok(world.rooms.rust_barleytrunk, "rust_barleytrunk room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleytrunk"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumtrunk sorghumtrunk", () => {
  assert.ok(world.rooms.rust_sorghumtrunk, "rust_sorghumtrunk room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumtrunk"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millettrunk millettrunk", () => {
  assert.ok(world.rooms.rust_millettrunk, "rust_millettrunk room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millettrunk"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corntrunk corntrunk", () => {
  assert.ok(world.rooms.rust_corntrunk, "rust_corntrunk room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corntrunk"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oattrunk oattrunk", () => {
  assert.ok(world.rooms.rust_oattrunk, "rust_oattrunk room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oattrunk"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumlocker durumlocker", () => {
  assert.ok(world.rooms.rust_durumlocker, "rust_durumlocker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumlocker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniolocker foniolocker", () => {
  assert.ok(world.rooms.rust_foniolocker, "rust_foniolocker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniolocker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefflocker tefflocker", () => {
  assert.ok(world.rooms.rust_tefflocker, "rust_tefflocker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefflocker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleylocker barleylocker", () => {
  assert.ok(world.rooms.rust_barleylocker, "rust_barleylocker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleylocker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumlocker sorghumlocker", () => {
  assert.ok(world.rooms.rust_sorghumlocker, "rust_sorghumlocker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumlocker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletlocker milletlocker", () => {
  assert.ok(world.rooms.rust_milletlocker, "rust_milletlocker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletlocker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornlocker cornlocker", () => {
  assert.ok(world.rooms.rust_cornlocker, "rust_cornlocker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornlocker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatlocker oatlocker", () => {
  assert.ok(world.rooms.rust_oatlocker, "rust_oatlocker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatlocker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumchest durumchest", () => {
  assert.ok(world.rooms.rust_durumchest, "rust_durumchest room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumchest"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniochest foniochest", () => {
  assert.ok(world.rooms.rust_foniochest, "rust_foniochest room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniochest"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffchest teffchest", () => {
  assert.ok(world.rooms.rust_teffchest, "rust_teffchest room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffchest"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleychest barleychest", () => {
  assert.ok(world.rooms.rust_barleychest, "rust_barleychest room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleychest"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumchest sorghumchest", () => {
  assert.ok(world.rooms.rust_sorghumchest, "rust_sorghumchest room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumchest"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletchest milletchest", () => {
  assert.ok(world.rooms.rust_milletchest, "rust_milletchest room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletchest"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornchest cornchest", () => {
  assert.ok(world.rooms.rust_cornchest, "rust_cornchest room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornchest"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatchest oatchest", () => {
  assert.ok(world.rooms.rust_oatchest, "rust_oatchest room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatchest"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumdrum durumdrum", () => {
  assert.ok(world.rooms.rust_durumdrum, "rust_durumdrum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumdrum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniodrum foniodrum", () => {
  assert.ok(world.rooms.rust_foniodrum, "rust_foniodrum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniodrum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffdrum teffdrum", () => {
  assert.ok(world.rooms.rust_teffdrum, "rust_teffdrum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffdrum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleydrum barleydrum", () => {
  assert.ok(world.rooms.rust_barleydrum, "rust_barleydrum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleydrum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumdrum sorghumdrum", () => {
  assert.ok(world.rooms.rust_sorghumdrum, "rust_sorghumdrum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumdrum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletdrum milletdrum", () => {
  assert.ok(world.rooms.rust_milletdrum, "rust_milletdrum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletdrum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corndrum corndrum", () => {
  assert.ok(world.rooms.rust_corndrum, "rust_corndrum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corndrum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatdrum oatdrum", () => {
  assert.ok(world.rooms.rust_oatdrum, "rust_oatdrum room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatdrum"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumcan durumcan", () => {
  assert.ok(world.rooms.rust_durumcan, "rust_durumcan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumcan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniocan foniocan", () => {
  assert.ok(world.rooms.rust_foniocan, "rust_foniocan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniocan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffcan teffcan", () => {
  assert.ok(world.rooms.rust_teffcan, "rust_teffcan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffcan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleycan barleycan", () => {
  assert.ok(world.rooms.rust_barleycan, "rust_barleycan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleycan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumcan sorghumcan", () => {
  assert.ok(world.rooms.rust_sorghumcan, "rust_sorghumcan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumcan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletcan milletcan", () => {
  assert.ok(world.rooms.rust_milletcan, "rust_milletcan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletcan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corncan corncan", () => {
  assert.ok(world.rooms.rust_corncan, "rust_corncan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corncan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatcan oatcan", () => {
  assert.ok(world.rooms.rust_oatcan, "rust_oatcan room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatcan"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumjar durumjar", () => {
  assert.ok(world.rooms.rust_durumjar, "rust_durumjar room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumjar"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniojar foniojar", () => {
  assert.ok(world.rooms.rust_foniojar, "rust_foniojar room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniojar"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffjar teffjar", () => {
  assert.ok(world.rooms.rust_teffjar, "rust_teffjar room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffjar"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyjar barleyjar", () => {
  assert.ok(world.rooms.rust_barleyjar, "rust_barleyjar room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyjar"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumjar sorghumjar", () => {
  assert.ok(world.rooms.rust_sorghumjar, "rust_sorghumjar room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumjar"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletjar milletjar", () => {
  assert.ok(world.rooms.rust_milletjar, "rust_milletjar room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletjar"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornjar cornjar", () => {
  assert.ok(world.rooms.rust_cornjar, "rust_cornjar room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornjar"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatjar oatjar", () => {
  assert.ok(world.rooms.rust_oatjar, "rust_oatjar room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatjar"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumtub durumtub", () => {
  assert.ok(world.rooms.rust_durumtub, "rust_durumtub room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumtub"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniotub foniotub", () => {
  assert.ok(world.rooms.rust_foniotub, "rust_foniotub room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniotub"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefftub tefftub", () => {
  assert.ok(world.rooms.rust_tefftub, "rust_tefftub room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefftub"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleytub barleytub", () => {
  assert.ok(world.rooms.rust_barleytub, "rust_barleytub room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleytub"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumtub sorghumtub", () => {
  assert.ok(world.rooms.rust_sorghumtub, "rust_sorghumtub room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumtub"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millettub millettub", () => {
  assert.ok(world.rooms.rust_millettub, "rust_millettub room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millettub"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corntub corntub", () => {
  assert.ok(world.rooms.rust_corntub, "rust_corntub room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corntub"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oattub oattub", () => {
  assert.ok(world.rooms.rust_oattub, "rust_oattub room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oattub"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumcask durumcask", () => {
  assert.ok(world.rooms.rust_durumcask, "rust_durumcask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumcask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniocask foniocask", () => {
  assert.ok(world.rooms.rust_foniocask, "rust_foniocask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniocask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffcask teffcask", () => {
  assert.ok(world.rooms.rust_teffcask, "rust_teffcask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffcask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleycask barleycask", () => {
  assert.ok(world.rooms.rust_barleycask, "rust_barleycask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleycask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumcask sorghumcask", () => {
  assert.ok(world.rooms.rust_sorghumcask, "rust_sorghumcask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumcask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletcask milletcask", () => {
  assert.ok(world.rooms.rust_milletcask, "rust_milletcask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletcask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corncask corncask", () => {
  assert.ok(world.rooms.rust_corncask, "rust_corncask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corncask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatcask oatcask", () => {
  assert.ok(world.rooms.rust_oatcask, "rust_oatcask room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatcask"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumvat durumvat", () => {
  assert.ok(world.rooms.rust_durumvat, "rust_durumvat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumvat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniovat foniovat", () => {
  assert.ok(world.rooms.rust_foniovat, "rust_foniovat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniovat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffvat teffvat", () => {
  assert.ok(world.rooms.rust_teffvat, "rust_teffvat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffvat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyvat barleyvat", () => {
  assert.ok(world.rooms.rust_barleyvat, "rust_barleyvat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyvat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumvat sorghumvat", () => {
  assert.ok(world.rooms.rust_sorghumvat, "rust_sorghumvat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumvat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletvat milletvat", () => {
  assert.ok(world.rooms.rust_milletvat, "rust_milletvat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletvat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornvat cornvat", () => {
  assert.ok(world.rooms.rust_cornvat, "rust_cornvat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornvat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatvat oatvat", () => {
  assert.ok(world.rooms.rust_oatvat, "rust_oatvat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatvat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumkeg durumkeg", () => {
  assert.ok(world.rooms.rust_durumkeg, "rust_durumkeg room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumkeg"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniokeg foniokeg", () => {
  assert.ok(world.rooms.rust_foniokeg, "rust_foniokeg room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniokeg"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffkeg teffkeg", () => {
  assert.ok(world.rooms.rust_teffkeg, "rust_teffkeg room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffkeg"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleykeg barleykeg", () => {
  assert.ok(world.rooms.rust_barleykeg, "rust_barleykeg room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleykeg"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumkeg sorghumkeg", () => {
  assert.ok(world.rooms.rust_sorghumkeg, "rust_sorghumkeg room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumkeg"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletkeg milletkeg", () => {
  assert.ok(world.rooms.rust_milletkeg, "rust_milletkeg room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletkeg"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornkeg cornkeg", () => {
  assert.ok(world.rooms.rust_cornkeg, "rust_cornkeg room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornkeg"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatkeg oatkeg", () => {
  assert.ok(world.rooms.rust_oatkeg, "rust_oatkeg room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatkeg"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumpail durumpail", () => {
  assert.ok(world.rooms.rust_durumpail, "rust_durumpail room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumpail"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniopail foniopail", () => {
  assert.ok(world.rooms.rust_foniopail, "rust_foniopail room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniopail"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffpail teffpail", () => {
  assert.ok(world.rooms.rust_teffpail, "rust_teffpail room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffpail"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleypail barleypail", () => {
  assert.ok(world.rooms.rust_barleypail, "rust_barleypail room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleypail"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumpail sorghumpail", () => {
  assert.ok(world.rooms.rust_sorghumpail, "rust_sorghumpail room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumpail"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletpail milletpail", () => {
  assert.ok(world.rooms.rust_milletpail, "rust_milletpail room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletpail"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornpail cornpail", () => {
  assert.ok(world.rooms.rust_cornpail, "rust_cornpail room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornpail"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatpail oatpail", () => {
  assert.ok(world.rooms.rust_oatpail, "rust_oatpail room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatpail"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumtin durumtin", () => {
  assert.ok(world.rooms.rust_durumtin, "rust_durumtin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumtin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniotin foniotin", () => {
  assert.ok(world.rooms.rust_foniotin, "rust_foniotin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniotin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefftin tefftin", () => {
  assert.ok(world.rooms.rust_tefftin, "rust_tefftin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefftin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleytin barleytin", () => {
  assert.ok(world.rooms.rust_barleytin, "rust_barleytin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleytin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumtin sorghumtin", () => {
  assert.ok(world.rooms.rust_sorghumtin, "rust_sorghumtin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumtin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millettin millettin", () => {
  assert.ok(world.rooms.rust_millettin, "rust_millettin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millettin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corntin corntin", () => {
  assert.ok(world.rooms.rust_corntin, "rust_corntin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corntin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oattin oattin", () => {
  assert.ok(world.rooms.rust_oattin, "rust_oattin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oattin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumcrate durumcrate", () => {
  assert.ok(world.rooms.rust_durumcrate, "rust_durumcrate room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumcrate"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniocrate foniocrate", () => {
  assert.ok(world.rooms.rust_foniocrate, "rust_foniocrate room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniocrate"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffcrate teffcrate", () => {
  assert.ok(world.rooms.rust_teffcrate, "rust_teffcrate room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffcrate"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleycrate barleycrate", () => {
  assert.ok(world.rooms.rust_barleycrate, "rust_barleycrate room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleycrate"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumcrate sorghumcrate", () => {
  assert.ok(world.rooms.rust_sorghumcrate, "rust_sorghumcrate room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumcrate"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletcrate milletcrate", () => {
  assert.ok(world.rooms.rust_milletcrate, "rust_milletcrate room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletcrate"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corncrate corncrate", () => {
  assert.ok(world.rooms.rust_corncrate, "rust_corncrate room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corncrate"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatcrate oatcrate", () => {
  assert.ok(world.rooms.rust_oatcrate, "rust_oatcrate room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatcrate"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbox durumbox", () => {
  assert.ok(world.rooms.rust_durumbox, "rust_durumbox room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbox"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobox foniobox", () => {
  assert.ok(world.rooms.rust_foniobox, "rust_foniobox room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobox"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbox teffbox", () => {
  assert.ok(world.rooms.rust_teffbox, "rust_teffbox room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbox"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybox barleybox", () => {
  assert.ok(world.rooms.rust_barleybox, "rust_barleybox room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybox"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbox sorghumbox", () => {
  assert.ok(world.rooms.rust_sorghumbox, "rust_sorghumbox room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbox"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbox milletbox", () => {
  assert.ok(world.rooms.rust_milletbox, "rust_milletbox room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbox"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbox cornbox", () => {
  assert.ok(world.rooms.rust_cornbox, "rust_cornbox room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbox"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbox oatbox", () => {
  assert.ok(world.rooms.rust_oatbox, "rust_oatbox room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbox"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbag durumbag", () => {
  assert.ok(world.rooms.rust_durumbag, "rust_durumbag room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbag"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobag foniobag", () => {
  assert.ok(world.rooms.rust_foniobag, "rust_foniobag room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobag"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbag teffbag", () => {
  assert.ok(world.rooms.rust_teffbag, "rust_teffbag room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbag"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybag barleybag", () => {
  assert.ok(world.rooms.rust_barleybag, "rust_barleybag room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybag"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbag sorghumbag", () => {
  assert.ok(world.rooms.rust_sorghumbag, "rust_sorghumbag room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbag"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbag milletbag", () => {
  assert.ok(world.rooms.rust_milletbag, "rust_milletbag room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbag"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbag cornbag", () => {
  assert.ok(world.rooms.rust_cornbag, "rust_cornbag room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbag"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbag oatbag", () => {
  assert.ok(world.rooms.rust_oatbag, "rust_oatbag room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbag"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbale durumbale", () => {
  assert.ok(world.rooms.rust_durumbale, "rust_durumbale room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbale"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobale foniobale", () => {
  assert.ok(world.rooms.rust_foniobale, "rust_foniobale room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobale"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbale teffbale", () => {
  assert.ok(world.rooms.rust_teffbale, "rust_teffbale room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbale"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybale barleybale", () => {
  assert.ok(world.rooms.rust_barleybale, "rust_barleybale room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybale"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbale sorghumbale", () => {
  assert.ok(world.rooms.rust_sorghumbale, "rust_sorghumbale room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbale"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbale milletbale", () => {
  assert.ok(world.rooms.rust_milletbale, "rust_milletbale room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbale"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbale cornbale", () => {
  assert.ok(world.rooms.rust_cornbale, "rust_cornbale room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbale"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbale oatbale", () => {
  assert.ok(world.rooms.rust_oatbale, "rust_oatbale room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbale"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumsack durumsack", () => {
  assert.ok(world.rooms.rust_durumsack, "rust_durumsack room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumsack"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniosack foniosack", () => {
  assert.ok(world.rooms.rust_foniosack, "rust_foniosack room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniosack"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffsack teffsack", () => {
  assert.ok(world.rooms.rust_teffsack, "rust_teffsack room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffsack"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleysack barleysack", () => {
  assert.ok(world.rooms.rust_barleysack, "rust_barleysack room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleysack"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumsack sorghumsack", () => {
  assert.ok(world.rooms.rust_sorghumsack, "rust_sorghumsack room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumsack"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletsack milletsack", () => {
  assert.ok(world.rooms.rust_milletsack, "rust_milletsack room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletsack"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornsack cornsack", () => {
  assert.ok(world.rooms.rust_cornsack, "rust_cornsack room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornsack"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatsack oatsack", () => {
  assert.ok(world.rooms.rust_oatsack, "rust_oatsack room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatsack"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumcrib durumcrib", () => {
  assert.ok(world.rooms.rust_durumcrib, "rust_durumcrib room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumcrib"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniocrib foniocrib", () => {
  assert.ok(world.rooms.rust_foniocrib, "rust_foniocrib room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniocrib"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffcrib teffcrib", () => {
  assert.ok(world.rooms.rust_teffcrib, "rust_teffcrib room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffcrib"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleycrib barleycrib", () => {
  assert.ok(world.rooms.rust_barleycrib, "rust_barleycrib room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleycrib"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumcrib sorghumcrib", () => {
  assert.ok(world.rooms.rust_sorghumcrib, "rust_sorghumcrib room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumcrib"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletcrib milletcrib", () => {
  assert.ok(world.rooms.rust_milletcrib, "rust_milletcrib room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletcrib"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corncrib corncrib", () => {
  assert.ok(world.rooms.rust_corncrib, "rust_corncrib room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corncrib"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatcrib oatcrib", () => {
  assert.ok(world.rooms.rust_oatcrib, "rust_oatcrib room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatcrib"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumloft durumloft", () => {
  assert.ok(world.rooms.rust_durumloft, "rust_durumloft room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumloft"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioloft fonioloft", () => {
  assert.ok(world.rooms.rust_fonioloft, "rust_fonioloft room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioloft"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffloft teffloft", () => {
  assert.ok(world.rooms.rust_teffloft, "rust_teffloft room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffloft"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyloft barleyloft", () => {
  assert.ok(world.rooms.rust_barleyloft, "rust_barleyloft room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyloft"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumloft sorghumloft", () => {
  assert.ok(world.rooms.rust_sorghumloft, "rust_sorghumloft room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumloft"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletloft milletloft", () => {
  assert.ok(world.rooms.rust_milletloft, "rust_milletloft room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletloft"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornloft cornloft", () => {
  assert.ok(world.rooms.rust_cornloft, "rust_cornloft room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornloft"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatloft oatloft", () => {
  assert.ok(world.rooms.rust_oatloft, "rust_oatloft room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatloft"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durummill durummill", () => {
  assert.ok(world.rooms.rust_durummill, "rust_durummill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durummill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniomill foniomill", () => {
  assert.ok(world.rooms.rust_foniomill, "rust_foniomill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniomill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffmill teffmill", () => {
  assert.ok(world.rooms.rust_teffmill, "rust_teffmill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffmill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleymill barleymill", () => {
  assert.ok(world.rooms.rust_barleymill, "rust_barleymill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleymill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghummill sorghummill", () => {
  assert.ok(world.rooms.rust_sorghummill, "rust_sorghummill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghummill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletmill milletmill", () => {
  assert.ok(world.rooms.rust_milletmill, "rust_milletmill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletmill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornmill cornmill", () => {
  assert.ok(world.rooms.rust_cornmill, "rust_cornmill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornmill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatmill oatmill", () => {
  assert.ok(world.rooms.rust_oatmill, "rust_oatmill room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatmill"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbin durumbin", () => {
  assert.ok(world.rooms.rust_durumbin, "rust_durumbin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobin foniobin", () => {
  assert.ok(world.rooms.rust_foniobin, "rust_foniobin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbin teffbin", () => {
  assert.ok(world.rooms.rust_teffbin, "rust_teffbin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybin barleybin", () => {
  assert.ok(world.rooms.rust_barleybin, "rust_barleybin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbin sorghumbin", () => {
  assert.ok(world.rooms.rust_sorghumbin, "rust_sorghumbin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbin milletbin", () => {
  assert.ok(world.rooms.rust_milletbin, "rust_milletbin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbin cornbin", () => {
  assert.ok(world.rooms.rust_cornbin, "rust_cornbin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbin oatbin", () => {
  assert.ok(world.rooms.rust_oatbin, "rust_oatbin room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbin"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumrun durumrun", () => {
  assert.ok(world.rooms.rust_durumrun, "rust_durumrun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumrun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniorun foniorun", () => {
  assert.ok(world.rooms.rust_foniorun, "rust_foniorun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniorun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffrun teffrun", () => {
  assert.ok(world.rooms.rust_teffrun, "rust_teffrun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffrun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyrun barleyrun", () => {
  assert.ok(world.rooms.rust_barleyrun, "rust_barleyrun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyrun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumrun sorghumrun", () => {
  assert.ok(world.rooms.rust_sorghumrun, "rust_sorghumrun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumrun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletrun milletrun", () => {
  assert.ok(world.rooms.rust_milletrun, "rust_milletrun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletrun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornrun cornrun", () => {
  assert.ok(world.rooms.rust_cornrun, "rust_cornrun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornrun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatrun oatrun", () => {
  assert.ok(world.rooms.rust_oatrun, "rust_oatrun room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatrun"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumlot durumlot", () => {
  assert.ok(world.rooms.rust_durumlot, "rust_durumlot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumlot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniolot foniolot", () => {
  assert.ok(world.rooms.rust_foniolot, "rust_foniolot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniolot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefflot tefflot", () => {
  assert.ok(world.rooms.rust_tefflot, "rust_tefflot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefflot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleylot barleylot", () => {
  assert.ok(world.rooms.rust_barleylot, "rust_barleylot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleylot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumlot sorghumlot", () => {
  assert.ok(world.rooms.rust_sorghumlot, "rust_sorghumlot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumlot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletlot milletlot", () => {
  assert.ok(world.rooms.rust_milletlot, "rust_milletlot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletlot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornlot cornlot", () => {
  assert.ok(world.rooms.rust_cornlot, "rust_cornlot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornlot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatlot oatlot", () => {
  assert.ok(world.rooms.rust_oatlot, "rust_oatlot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatlot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumcut durumcut", () => {
  assert.ok(world.rooms.rust_durumcut, "rust_durumcut room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumcut"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniocut foniocut", () => {
  assert.ok(world.rooms.rust_foniocut, "rust_foniocut room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniocut"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffcut teffcut", () => {
  assert.ok(world.rooms.rust_teffcut, "rust_teffcut room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffcut"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleycut barleycut", () => {
  assert.ok(world.rooms.rust_barleycut, "rust_barleycut room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleycut"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumcut sorghumcut", () => {
  assert.ok(world.rooms.rust_sorghumcut, "rust_sorghumcut room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumcut"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletcut milletcut", () => {
  assert.ok(world.rooms.rust_milletcut, "rust_milletcut room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletcut"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corncut corncut", () => {
  assert.ok(world.rooms.rust_corncut, "rust_corncut room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corncut"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatcut oatcut", () => {
  assert.ok(world.rooms.rust_oatcut, "rust_oatcut room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatcut"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumgrade durumgrade", () => {
  assert.ok(world.rooms.rust_durumgrade, "rust_durumgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniograde foniograde", () => {
  assert.ok(world.rooms.rust_foniograde, "rust_foniograde room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniograde"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffgrade teffgrade", () => {
  assert.ok(world.rooms.rust_teffgrade, "rust_teffgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleygrade barleygrade", () => {
  assert.ok(world.rooms.rust_barleygrade, "rust_barleygrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleygrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumgrade sorghumgrade", () => {
  assert.ok(world.rooms.rust_sorghumgrade, "rust_sorghumgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletgrade milletgrade", () => {
  assert.ok(world.rooms.rust_milletgrade, "rust_milletgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corngrade corngrade", () => {
  assert.ok(world.rooms.rust_corngrade, "rust_corngrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corngrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatgrade oatgrade", () => {
  assert.ok(world.rooms.rust_oatgrade, "rust_oatgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumchoice durumchoice", () => {
  assert.ok(world.rooms.rust_durumchoice, "rust_durumchoice room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumchoice"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniochoice foniochoice", () => {
  assert.ok(world.rooms.rust_foniochoice, "rust_foniochoice room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniochoice"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffchoice teffchoice", () => {
  assert.ok(world.rooms.rust_teffchoice, "rust_teffchoice room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffchoice"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleychoice barleychoice", () => {
  assert.ok(world.rooms.rust_barleychoice, "rust_barleychoice room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleychoice"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumchoice sorghumchoice", () => {
  assert.ok(world.rooms.rust_sorghumchoice, "rust_sorghumchoice room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumchoice"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletchoice milletchoice", () => {
  assert.ok(world.rooms.rust_milletchoice, "rust_milletchoice room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletchoice"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornchoice cornchoice", () => {
  assert.ok(world.rooms.rust_cornchoice, "rust_cornchoice room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornchoice"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatchoice oatchoice", () => {
  assert.ok(world.rooms.rust_oatchoice, "rust_oatchoice room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatchoice"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumprime durumprime", () => {
  assert.ok(world.rooms.rust_durumprime, "rust_durumprime room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumprime"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioprime fonioprime", () => {
  assert.ok(world.rooms.rust_fonioprime, "rust_fonioprime room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioprime"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffprime teffprime", () => {
  assert.ok(world.rooms.rust_teffprime, "rust_teffprime room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffprime"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyprime barleyprime", () => {
  assert.ok(world.rooms.rust_barleyprime, "rust_barleyprime room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyprime"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumprime sorghumprime", () => {
  assert.ok(world.rooms.rust_sorghumprime, "rust_sorghumprime room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumprime"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletprime milletprime", () => {
  assert.ok(world.rooms.rust_milletprime, "rust_milletprime room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletprime"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornprime cornprime", () => {
  assert.ok(world.rooms.rust_cornprime, "rust_cornprime room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornprime"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatprime oatprime", () => {
  assert.ok(world.rooms.rust_oatprime, "rust_oatprime room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatprime"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumselect durumselect", () => {
  assert.ok(world.rooms.rust_durumselect, "rust_durumselect room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumselect"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioselect fonioselect", () => {
  assert.ok(world.rooms.rust_fonioselect, "rust_fonioselect room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioselect"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffselect teffselect", () => {
  assert.ok(world.rooms.rust_teffselect, "rust_teffselect room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffselect"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyselect barleyselect", () => {
  assert.ok(world.rooms.rust_barleyselect, "rust_barleyselect room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyselect"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumselect sorghumselect", () => {
  assert.ok(world.rooms.rust_sorghumselect, "rust_sorghumselect room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumselect"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletselect milletselect", () => {
  assert.ok(world.rooms.rust_milletselect, "rust_milletselect room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletselect"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornselect cornselect", () => {
  assert.ok(world.rooms.rust_cornselect, "rust_cornselect room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornselect"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatselect oatselect", () => {
  assert.ok(world.rooms.rust_oatselect, "rust_oatselect room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatselect"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumstandard durumstandard", () => {
  assert.ok(world.rooms.rust_durumstandard, "rust_durumstandard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumstandard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniostandard foniostandard", () => {
  assert.ok(world.rooms.rust_foniostandard, "rust_foniostandard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniostandard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffstandard teffstandard", () => {
  assert.ok(world.rooms.rust_teffstandard, "rust_teffstandard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffstandard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleystandard barleystandard", () => {
  assert.ok(world.rooms.rust_barleystandard, "rust_barleystandard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleystandard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumstandard sorghumstandard", () => {
  assert.ok(world.rooms.rust_sorghumstandard, "rust_sorghumstandard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumstandard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletstandard milletstandard", () => {
  assert.ok(world.rooms.rust_milletstandard, "rust_milletstandard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletstandard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornstandard cornstandard", () => {
  assert.ok(world.rooms.rust_cornstandard, "rust_cornstandard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornstandard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatstandard oatstandard", () => {
  assert.ok(world.rooms.rust_oatstandard, "rust_oatstandard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatstandard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumgeneral durumgeneral", () => {
  assert.ok(world.rooms.rust_durumgeneral, "rust_durumgeneral room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumgeneral"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniogeneral foniogeneral", () => {
  assert.ok(world.rooms.rust_foniogeneral, "rust_foniogeneral room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniogeneral"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffgeneral teffgeneral", () => {
  assert.ok(world.rooms.rust_teffgeneral, "rust_teffgeneral room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffgeneral"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleygeneral barleygeneral", () => {
  assert.ok(world.rooms.rust_barleygeneral, "rust_barleygeneral room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleygeneral"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumgeneral sorghumgeneral", () => {
  assert.ok(world.rooms.rust_sorghumgeneral, "rust_sorghumgeneral room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumgeneral"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletgeneral milletgeneral", () => {
  assert.ok(world.rooms.rust_milletgeneral, "rust_milletgeneral room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletgeneral"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corngeneral corngeneral", () => {
  assert.ok(world.rooms.rust_corngeneral, "rust_corngeneral room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corngeneral"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatgeneral oatgeneral", () => {
  assert.ok(world.rooms.rust_oatgeneral, "rust_oatgeneral room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatgeneral"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumweak durumweak", () => {
  assert.ok(world.rooms.rust_durumweak, "rust_durumweak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumweak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioweak fonioweak", () => {
  assert.ok(world.rooms.rust_fonioweak, "rust_fonioweak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioweak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffweak teffweak", () => {
  assert.ok(world.rooms.rust_teffweak, "rust_teffweak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffweak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyweak barleyweak", () => {
  assert.ok(world.rooms.rust_barleyweak, "rust_barleyweak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyweak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumweak sorghumweak", () => {
  assert.ok(world.rooms.rust_sorghumweak, "rust_sorghumweak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumweak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletweak milletweak", () => {
  assert.ok(world.rooms.rust_milletweak, "rust_milletweak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletweak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornweak cornweak", () => {
  assert.ok(world.rooms.rust_cornweak, "rust_cornweak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornweak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatweak oatweak", () => {
  assert.ok(world.rooms.rust_oatweak, "rust_oatweak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatweak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumstrong durumstrong", () => {
  assert.ok(world.rooms.rust_durumstrong, "rust_durumstrong room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumstrong"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniostrong foniostrong", () => {
  assert.ok(world.rooms.rust_foniostrong, "rust_foniostrong room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniostrong"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffstrong teffstrong", () => {
  assert.ok(world.rooms.rust_teffstrong, "rust_teffstrong room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffstrong"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleystrong barleystrong", () => {
  assert.ok(world.rooms.rust_barleystrong, "rust_barleystrong room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleystrong"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumstrong sorghumstrong", () => {
  assert.ok(world.rooms.rust_sorghumstrong, "rust_sorghumstrong room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumstrong"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletstrong milletstrong", () => {
  assert.ok(world.rooms.rust_milletstrong, "rust_milletstrong room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletstrong"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornstrong cornstrong", () => {
  assert.ok(world.rooms.rust_cornstrong, "rust_cornstrong room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornstrong"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatstrong oatstrong", () => {
  assert.ok(world.rooms.rust_oatstrong, "rust_oatstrong room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatstrong"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumfamily durumfamily", () => {
  assert.ok(world.rooms.rust_durumfamily, "rust_durumfamily room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumfamily"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniofamily foniofamily", () => {
  assert.ok(world.rooms.rust_foniofamily, "rust_foniofamily room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniofamily"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefffamily tefffamily", () => {
  assert.ok(world.rooms.rust_tefffamily, "rust_tefffamily room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefffamily"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyfamily barleyfamily", () => {
  assert.ok(world.rooms.rust_barleyfamily, "rust_barleyfamily room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyfamily"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumfamily sorghumfamily", () => {
  assert.ok(world.rooms.rust_sorghumfamily, "rust_sorghumfamily room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumfamily"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletfamily milletfamily", () => {
  assert.ok(world.rooms.rust_milletfamily, "rust_milletfamily room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletfamily"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornfamily cornfamily", () => {
  assert.ok(world.rooms.rust_cornfamily, "rust_cornfamily room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornfamily"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatfamily oatfamily", () => {
  assert.ok(world.rooms.rust_oatfamily, "rust_oatfamily room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatfamily"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumhousehold durumhousehold", () => {
  assert.ok(world.rooms.rust_durumhousehold, "rust_durumhousehold room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumhousehold"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniohousehold foniohousehold", () => {
  assert.ok(world.rooms.rust_foniohousehold, "rust_foniohousehold room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniohousehold"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffhousehold teffhousehold", () => {
  assert.ok(world.rooms.rust_teffhousehold, "rust_teffhousehold room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffhousehold"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyhousehold barleyhousehold", () => {
  assert.ok(world.rooms.rust_barleyhousehold, "rust_barleyhousehold room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyhousehold"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumhousehold sorghumhousehold", () => {
  assert.ok(world.rooms.rust_sorghumhousehold, "rust_sorghumhousehold room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumhousehold"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millethousehold millethousehold", () => {
  assert.ok(world.rooms.rust_millethousehold, "rust_millethousehold room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millethousehold"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornhousehold cornhousehold", () => {
  assert.ok(world.rooms.rust_cornhousehold, "rust_cornhousehold room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornhousehold"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oathousehold oathousehold", () => {
  assert.ok(world.rooms.rust_oathousehold, "rust_oathousehold room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oathousehold"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbolted durumbolted", () => {
  assert.ok(world.rooms.rust_durumbolted, "rust_durumbolted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbolted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobolted foniobolted", () => {
  assert.ok(world.rooms.rust_foniobolted, "rust_foniobolted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobolted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbolted teffbolted", () => {
  assert.ok(world.rooms.rust_teffbolted, "rust_teffbolted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbolted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybolted barleybolted", () => {
  assert.ok(world.rooms.rust_barleybolted, "rust_barleybolted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybolted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbolted sorghumbolted", () => {
  assert.ok(world.rooms.rust_sorghumbolted, "rust_sorghumbolted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbolted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbolted milletbolted", () => {
  assert.ok(world.rooms.rust_milletbolted, "rust_milletbolted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbolted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbolted cornbolted", () => {
  assert.ok(world.rooms.rust_cornbolted, "rust_cornbolted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbolted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbolted oatbolted", () => {
  assert.ok(world.rooms.rust_oatbolted, "rust_oatbolted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbolted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumsifted durumsifted", () => {
  assert.ok(world.rooms.rust_durumsifted, "rust_durumsifted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumsifted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniosifted foniosifted", () => {
  assert.ok(world.rooms.rust_foniosifted, "rust_foniosifted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniosifted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffsifted teffsifted", () => {
  assert.ok(world.rooms.rust_teffsifted, "rust_teffsifted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffsifted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleysifted barleysifted", () => {
  assert.ok(world.rooms.rust_barleysifted, "rust_barleysifted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleysifted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumsifted sorghumsifted", () => {
  assert.ok(world.rooms.rust_sorghumsifted, "rust_sorghumsifted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumsifted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletsifted milletsifted", () => {
  assert.ok(world.rooms.rust_milletsifted, "rust_milletsifted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletsifted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornsifted cornsifted", () => {
  assert.ok(world.rooms.rust_cornsifted, "rust_cornsifted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornsifted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatsifted oatsifted", () => {
  assert.ok(world.rooms.rust_oatsifted, "rust_oatsifted room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatsifted"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumplain durumplain", () => {
  assert.ok(world.rooms.rust_durumplain, "rust_durumplain room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumplain"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioplain fonioplain", () => {
  assert.ok(world.rooms.rust_fonioplain, "rust_fonioplain room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioplain"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffplain teffplain", () => {
  assert.ok(world.rooms.rust_teffplain, "rust_teffplain room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffplain"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyplain barleyplain", () => {
  assert.ok(world.rooms.rust_barleyplain, "rust_barleyplain room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyplain"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumplain sorghumplain", () => {
  assert.ok(world.rooms.rust_sorghumplain, "rust_sorghumplain room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumplain"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletplain milletplain", () => {
  assert.ok(world.rooms.rust_milletplain, "rust_milletplain room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletplain"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornplain cornplain", () => {
  assert.ok(world.rooms.rust_cornplain, "rust_cornplain room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornplain"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatplain oatplain", () => {
  assert.ok(world.rooms.rust_oatplain, "rust_oatplain room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatplain"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumallpurpose durumallpurpose", () => {
  assert.ok(world.rooms.rust_durumallpurpose, "rust_durumallpurpose room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumallpurpose"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioallpurpose fonioallpurpose", () => {
  assert.ok(world.rooms.rust_fonioallpurpose, "rust_fonioallpurpose room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioallpurpose"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffallpurpose teffallpurpose", () => {
  assert.ok(world.rooms.rust_teffallpurpose, "rust_teffallpurpose room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffallpurpose"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyallpurpose barleyallpurpose", () => {
  assert.ok(world.rooms.rust_barleyallpurpose, "rust_barleyallpurpose room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyallpurpose"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumallpurpose sorghumallpurpose", () => {
  assert.ok(world.rooms.rust_sorghumallpurpose, "rust_sorghumallpurpose room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumallpurpose"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletallpurpose milletallpurpose", () => {
  assert.ok(world.rooms.rust_milletallpurpose, "rust_milletallpurpose room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletallpurpose"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornallpurpose cornallpurpose", () => {
  assert.ok(world.rooms.rust_cornallpurpose, "rust_cornallpurpose room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornallpurpose"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatallpurpose oatallpurpose", () => {
  assert.ok(world.rooms.rust_oatallpurpose, "rust_oatallpurpose room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatallpurpose"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryeallpurpose ryeallpurpose", () => {
  assert.ok(world.rooms.rust_ryeallpurpose, "rust_ryeallpurpose room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryeallpurpose"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatallpurpose wheatallpurpose", () => {
  assert.ok(world.rooms.rust_wheatallpurpose, "rust_wheatallpurpose room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatallpurpose"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbaker durumbaker", () => {
  assert.ok(world.rooms.rust_durumbaker, "rust_durumbaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobaker foniobaker", () => {
  assert.ok(world.rooms.rust_foniobaker, "rust_foniobaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbaker teffbaker", () => {
  assert.ok(world.rooms.rust_teffbaker, "rust_teffbaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybaker barleybaker", () => {
  assert.ok(world.rooms.rust_barleybaker, "rust_barleybaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbaker sorghumbaker", () => {
  assert.ok(world.rooms.rust_sorghumbaker, "rust_sorghumbaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbaker milletbaker", () => {
  assert.ok(world.rooms.rust_milletbaker, "rust_milletbaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbaker cornbaker", () => {
  assert.ok(world.rooms.rust_cornbaker, "rust_cornbaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbaker oatbaker", () => {
  assert.ok(world.rooms.rust_oatbaker, "rust_oatbaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryebaker ryebaker", () => {
  assert.ok(world.rooms.rust_ryebaker, "rust_ryebaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryebaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatbaker wheatbaker", () => {
  assert.ok(world.rooms.rust_wheatbaker, "rust_wheatbaker room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatbaker"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumstraight durumstraight", () => {
  assert.ok(world.rooms.rust_durumstraight, "rust_durumstraight room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumstraight"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniostraight foniostraight", () => {
  assert.ok(world.rooms.rust_foniostraight, "rust_foniostraight room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniostraight"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffstraight teffstraight", () => {
  assert.ok(world.rooms.rust_teffstraight, "rust_teffstraight room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffstraight"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleystraight barleystraight", () => {
  assert.ok(world.rooms.rust_barleystraight, "rust_barleystraight room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleystraight"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumstraight sorghumstraight", () => {
  assert.ok(world.rooms.rust_sorghumstraight, "rust_sorghumstraight room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumstraight"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletstraight milletstraight", () => {
  assert.ok(world.rooms.rust_milletstraight, "rust_milletstraight room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletstraight"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornstraight cornstraight", () => {
  assert.ok(world.rooms.rust_cornstraight, "rust_cornstraight room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornstraight"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatstraight oatstraight", () => {
  assert.ok(world.rooms.rust_oatstraight, "rust_oatstraight room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatstraight"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryestraight ryestraight", () => {
  assert.ok(world.rooms.rust_ryestraight, "rust_ryestraight room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryestraight"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatstraight wheatstraight", () => {
  assert.ok(world.rooms.rust_wheatstraight, "rust_wheatstraight room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatstraight"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumsecondclear durumsecondclear", () => {
  assert.ok(world.rooms.rust_durumsecondclear, "rust_durumsecondclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumsecondclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniosecondclear foniosecondclear", () => {
  assert.ok(world.rooms.rust_foniosecondclear, "rust_foniosecondclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniosecondclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffsecondclear teffsecondclear", () => {
  assert.ok(world.rooms.rust_teffsecondclear, "rust_teffsecondclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffsecondclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleysecondclear barleysecondclear", () => {
  assert.ok(world.rooms.rust_barleysecondclear, "rust_barleysecondclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleysecondclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumsecondclear sorghumsecondclear", () => {
  assert.ok(world.rooms.rust_sorghumsecondclear, "rust_sorghumsecondclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumsecondclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletsecondclear milletsecondclear", () => {
  assert.ok(world.rooms.rust_milletsecondclear, "rust_milletsecondclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletsecondclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornsecondclear cornsecondclear", () => {
  assert.ok(world.rooms.rust_cornsecondclear, "rust_cornsecondclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornsecondclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatsecondclear oatsecondclear", () => {
  assert.ok(world.rooms.rust_oatsecondclear, "rust_oatsecondclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatsecondclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryesecondclear ryesecondclear", () => {
  assert.ok(world.rooms.rust_ryesecondclear, "rust_ryesecondclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryesecondclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatsecondclear wheatsecondclear", () => {
  assert.ok(world.rooms.rust_wheatsecondclear, "rust_wheatsecondclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatsecondclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumfirstclear durumfirstclear", () => {
  assert.ok(world.rooms.rust_durumfirstclear, "rust_durumfirstclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumfirstclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniofirstclear foniofirstclear", () => {
  assert.ok(world.rooms.rust_foniofirstclear, "rust_foniofirstclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniofirstclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefffirstclear tefffirstclear", () => {
  assert.ok(world.rooms.rust_tefffirstclear, "rust_tefffirstclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefffirstclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyfirstclear barleyfirstclear", () => {
  assert.ok(world.rooms.rust_barleyfirstclear, "rust_barleyfirstclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyfirstclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumfirstclear sorghumfirstclear", () => {
  assert.ok(world.rooms.rust_sorghumfirstclear, "rust_sorghumfirstclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumfirstclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletfirstclear milletfirstclear", () => {
  assert.ok(world.rooms.rust_milletfirstclear, "rust_milletfirstclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletfirstclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornfirstclear cornfirstclear", () => {
  assert.ok(world.rooms.rust_cornfirstclear, "rust_cornfirstclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornfirstclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatfirstclear oatfirstclear", () => {
  assert.ok(world.rooms.rust_oatfirstclear, "rust_oatfirstclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatfirstclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryefirstclear ryefirstclear", () => {
  assert.ok(world.rooms.rust_ryefirstclear, "rust_ryefirstclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryefirstclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatfirstclear wheatfirstclear", () => {
  assert.ok(world.rooms.rust_wheatfirstclear, "rust_wheatfirstclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatfirstclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumpatent durumpatent", () => {
  assert.ok(world.rooms.rust_durumpatent, "rust_durumpatent room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumpatent"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniopatent foniopatent", () => {
  assert.ok(world.rooms.rust_foniopatent, "rust_foniopatent room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniopatent"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffpatent teffpatent", () => {
  assert.ok(world.rooms.rust_teffpatent, "rust_teffpatent room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffpatent"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleypatent barleypatent", () => {
  assert.ok(world.rooms.rust_barleypatent, "rust_barleypatent room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleypatent"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumpatent sorghumpatent", () => {
  assert.ok(world.rooms.rust_sorghumpatent, "rust_sorghumpatent room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumpatent"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletpatent milletpatent", () => {
  assert.ok(world.rooms.rust_milletpatent, "rust_milletpatent room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletpatent"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornpatent cornpatent", () => {
  assert.ok(world.rooms.rust_cornpatent, "rust_cornpatent room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornpatent"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatpatent oatpatent", () => {
  assert.ok(world.rooms.rust_oatpatent, "rust_oatpatent room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatpatent"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryepatent ryepatent", () => {
  assert.ok(world.rooms.rust_ryepatent, "rust_ryepatent room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryepatent"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatpatent wheatpatent", () => {
  assert.ok(world.rooms.rust_wheatpatent, "rust_wheatpatent room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatpatent"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbreak durumbreak", () => {
  assert.ok(world.rooms.rust_durumbreak, "rust_durumbreak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbreak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobreak foniobreak", () => {
  assert.ok(world.rooms.rust_foniobreak, "rust_foniobreak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobreak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbreak teffbreak", () => {
  assert.ok(world.rooms.rust_teffbreak, "rust_teffbreak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbreak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybreak barleybreak", () => {
  assert.ok(world.rooms.rust_barleybreak, "rust_barleybreak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybreak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbreak sorghumbreak", () => {
  assert.ok(world.rooms.rust_sorghumbreak, "rust_sorghumbreak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbreak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbreak milletbreak", () => {
  assert.ok(world.rooms.rust_milletbreak, "rust_milletbreak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbreak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbreak cornbreak", () => {
  assert.ok(world.rooms.rust_cornbreak, "rust_cornbreak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbreak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbreak oatbreak", () => {
  assert.ok(world.rooms.rust_oatbreak, "rust_oatbreak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbreak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryebreak ryebreak", () => {
  assert.ok(world.rooms.rust_ryebreak, "rust_ryebreak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryebreak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatbreak wheatbreak", () => {
  assert.ok(world.rooms.rust_wheatbreak, "rust_wheatbreak room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatbreak"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumlowgrade durumlowgrade", () => {
  assert.ok(world.rooms.rust_durumlowgrade, "rust_durumlowgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumlowgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniolowgrade foniolowgrade", () => {
  assert.ok(world.rooms.rust_foniolowgrade, "rust_foniolowgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniolowgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefflowgrade tefflowgrade", () => {
  assert.ok(world.rooms.rust_tefflowgrade, "rust_tefflowgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefflowgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleylowgrade barleylowgrade", () => {
  assert.ok(world.rooms.rust_barleylowgrade, "rust_barleylowgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleylowgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumlowgrade sorghumlowgrade", () => {
  assert.ok(world.rooms.rust_sorghumlowgrade, "rust_sorghumlowgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumlowgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletlowgrade milletlowgrade", () => {
  assert.ok(world.rooms.rust_milletlowgrade, "rust_milletlowgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletlowgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornlowgrade cornlowgrade", () => {
  assert.ok(world.rooms.rust_cornlowgrade, "rust_cornlowgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornlowgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatlowgrade oatlowgrade", () => {
  assert.ok(world.rooms.rust_oatlowgrade, "rust_oatlowgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatlowgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryelowgrade ryelowgrade", () => {
  assert.ok(world.rooms.rust_ryelowgrade, "rust_ryelowgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryelowgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatlowgrade wheatlowgrade", () => {
  assert.ok(world.rooms.rust_wheatlowgrade, "rust_wheatlowgrade room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatlowgrade"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumclear durumclear", () => {
  assert.ok(world.rooms.rust_durumclear, "rust_durumclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioclear fonioclear", () => {
  assert.ok(world.rooms.rust_fonioclear, "rust_fonioclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffclear teffclear", () => {
  assert.ok(world.rooms.rust_teffclear, "rust_teffclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyclear barleyclear", () => {
  assert.ok(world.rooms.rust_barleyclear, "rust_barleyclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumclear sorghumclear", () => {
  assert.ok(world.rooms.rust_sorghumclear, "rust_sorghumclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletclear milletclear", () => {
  assert.ok(world.rooms.rust_milletclear, "rust_milletclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornclear cornclear", () => {
  assert.ok(world.rooms.rust_cornclear, "rust_cornclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatclear oatclear", () => {
  assert.ok(world.rooms.rust_oatclear, "rust_oatclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryeclear ryeclear", () => {
  assert.ok(world.rooms.rust_ryeclear, "rust_ryeclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryeclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatclear wheatclear", () => {
  assert.ok(world.rooms.rust_wheatclear, "rust_wheatclear room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatclear"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumreddog durumreddog", () => {
  assert.ok(world.rooms.rust_durumreddog, "rust_durumreddog room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumreddog"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioreddog fonioreddog", () => {
  assert.ok(world.rooms.rust_fonioreddog, "rust_fonioreddog room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioreddog"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffreddog teffreddog", () => {
  assert.ok(world.rooms.rust_teffreddog, "rust_teffreddog room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffreddog"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyreddog barleyreddog", () => {
  assert.ok(world.rooms.rust_barleyreddog, "rust_barleyreddog room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyreddog"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumreddog sorghumreddog", () => {
  assert.ok(world.rooms.rust_sorghumreddog, "rust_sorghumreddog room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumreddog"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletreddog milletreddog", () => {
  assert.ok(world.rooms.rust_milletreddog, "rust_milletreddog room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletreddog"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornreddog cornreddog", () => {
  assert.ok(world.rooms.rust_cornreddog, "rust_cornreddog room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornreddog"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatreddog oatreddog", () => {
  assert.ok(world.rooms.rust_oatreddog, "rust_oatreddog room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatreddog"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryereddog ryereddog", () => {
  assert.ok(world.rooms.rust_ryereddog, "rust_ryereddog room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryereddog"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatreddog wheatreddog", () => {
  assert.ok(world.rooms.rust_wheatreddog, "rust_wheatreddog room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatreddog"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumoffal durumoffal", () => {
  assert.ok(world.rooms.rust_durumoffal, "rust_durumoffal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumoffal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniooffal foniooffal", () => {
  assert.ok(world.rooms.rust_foniooffal, "rust_foniooffal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniooffal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffoffal teffoffal", () => {
  assert.ok(world.rooms.rust_teffoffal, "rust_teffoffal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffoffal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyoffal barleyoffal", () => {
  assert.ok(world.rooms.rust_barleyoffal, "rust_barleyoffal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyoffal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumoffal sorghumoffal", () => {
  assert.ok(world.rooms.rust_sorghumoffal, "rust_sorghumoffal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumoffal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletoffal milletoffal", () => {
  assert.ok(world.rooms.rust_milletoffal, "rust_milletoffal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletoffal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornoffal cornoffal", () => {
  assert.ok(world.rooms.rust_cornoffal, "rust_cornoffal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornoffal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatoffal oatoffal", () => {
  assert.ok(world.rooms.rust_oatoffal, "rust_oatoffal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatoffal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryeoffal ryeoffal", () => {
  assert.ok(world.rooms.rust_ryeoffal, "rust_ryeoffal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryeoffal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatoffal wheatoffal", () => {
  assert.ok(world.rooms.rust_wheatoffal, "rust_wheatoffal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatoffal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumscreenings durumscreenings", () => {
  assert.ok(world.rooms.rust_durumscreenings, "rust_durumscreenings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumscreenings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioscreenings fonioscreenings", () => {
  assert.ok(world.rooms.rust_fonioscreenings, "rust_fonioscreenings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioscreenings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffscreenings teffscreenings", () => {
  assert.ok(world.rooms.rust_teffscreenings, "rust_teffscreenings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffscreenings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyscreenings barleyscreenings", () => {
  assert.ok(world.rooms.rust_barleyscreenings, "rust_barleyscreenings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyscreenings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumscreenings sorghumscreenings", () => {
  assert.ok(world.rooms.rust_sorghumscreenings, "rust_sorghumscreenings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumscreenings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletscreenings milletscreenings", () => {
  assert.ok(world.rooms.rust_milletscreenings, "rust_milletscreenings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletscreenings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornscreenings cornscreenings", () => {
  assert.ok(world.rooms.rust_cornscreenings, "rust_cornscreenings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornscreenings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatscreenings oatscreenings", () => {
  assert.ok(world.rooms.rust_oatscreenings, "rust_oatscreenings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatscreenings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatscreenings wheatscreenings", () => {
  assert.ok(world.rooms.rust_wheatscreenings, "rust_wheatscreenings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatscreenings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumgermflake durumgermflake", () => {
  assert.ok(world.rooms.rust_durumgermflake, "rust_durumgermflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumgermflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniogermflake foniogermflake", () => {
  assert.ok(world.rooms.rust_foniogermflake, "rust_foniogermflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniogermflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffgermflake teffgermflake", () => {
  assert.ok(world.rooms.rust_teffgermflake, "rust_teffgermflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffgermflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleygermflake barleygermflake", () => {
  assert.ok(world.rooms.rust_barleygermflake, "rust_barleygermflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleygermflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumgermflake sorghumgermflake", () => {
  assert.ok(world.rooms.rust_sorghumgermflake, "rust_sorghumgermflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumgermflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletgermflake milletgermflake", () => {
  assert.ok(world.rooms.rust_milletgermflake, "rust_milletgermflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletgermflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-corngermflake corngermflake", () => {
  assert.ok(world.rooms.rust_corngermflake, "rust_corngermflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the corngermflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatgermflake oatgermflake", () => {
  assert.ok(world.rooms.rust_oatgermflake, "rust_oatgermflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatgermflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryegermflake ryegermflake", () => {
  assert.ok(world.rooms.rust_ryegermflake, "rust_ryegermflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryegermflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatgermflake wheatgermflake", () => {
  assert.ok(world.rooms.rust_wheatgermflake, "rust_wheatgermflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatgermflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumfeedflake durumfeedflake", () => {
  assert.ok(world.rooms.rust_durumfeedflake, "rust_durumfeedflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumfeedflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniofeedflake foniofeedflake", () => {
  assert.ok(world.rooms.rust_foniofeedflake, "rust_foniofeedflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniofeedflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefffeedflake tefffeedflake", () => {
  assert.ok(world.rooms.rust_tefffeedflake, "rust_tefffeedflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefffeedflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyfeedflake barleyfeedflake", () => {
  assert.ok(world.rooms.rust_barleyfeedflake, "rust_barleyfeedflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyfeedflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumfeedflake sorghumfeedflake", () => {
  assert.ok(world.rooms.rust_sorghumfeedflake, "rust_sorghumfeedflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumfeedflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletfeedflake milletfeedflake", () => {
  assert.ok(world.rooms.rust_milletfeedflake, "rust_milletfeedflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletfeedflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornfeedflake cornfeedflake", () => {
  assert.ok(world.rooms.rust_cornfeedflake, "rust_cornfeedflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornfeedflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatfeedflake oatfeedflake", () => {
  assert.ok(world.rooms.rust_oatfeedflake, "rust_oatfeedflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatfeedflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryefeedflake ryefeedflake", () => {
  assert.ok(world.rooms.rust_ryefeedflake, "rust_ryefeedflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryefeedflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatfeedflake wheatfeedflake", () => {
  assert.ok(world.rooms.rust_wheatfeedflake, "rust_wheatfeedflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatfeedflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumshorts durumshorts", () => {
  assert.ok(world.rooms.rust_durumshorts, "rust_durumshorts room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumshorts"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioshorts fonioshorts", () => {
  assert.ok(world.rooms.rust_fonioshorts, "rust_fonioshorts room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioshorts"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffshorts teffshorts", () => {
  assert.ok(world.rooms.rust_teffshorts, "rust_teffshorts room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffshorts"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyshorts barleyshorts", () => {
  assert.ok(world.rooms.rust_barleyshorts, "rust_barleyshorts room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyshorts"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumshorts sorghumshorts", () => {
  assert.ok(world.rooms.rust_sorghumshorts, "rust_sorghumshorts room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumshorts"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletshorts milletshorts", () => {
  assert.ok(world.rooms.rust_milletshorts, "rust_milletshorts room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletshorts"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatshorts oatshorts", () => {
  assert.ok(world.rooms.rust_oatshorts, "rust_oatshorts room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatshorts"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryeshorts ryeshorts", () => {
  assert.ok(world.rooms.rust_ryeshorts, "rust_ryeshorts room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryeshorts"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durummiddlings durummiddlings", () => {
  assert.ok(world.rooms.rust_durummiddlings, "rust_durummiddlings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durummiddlings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniomiddlings foniomiddlings", () => {
  assert.ok(world.rooms.rust_foniomiddlings, "rust_foniomiddlings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniomiddlings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffmiddlings teffmiddlings", () => {
  assert.ok(world.rooms.rust_teffmiddlings, "rust_teffmiddlings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffmiddlings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleymiddlings barleymiddlings", () => {
  assert.ok(world.rooms.rust_barleymiddlings, "rust_barleymiddlings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleymiddlings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghimmiddlings sorghimmiddlings", () => {
  assert.ok(world.rooms.rust_sorghimmiddlings, "rust_sorghimmiddlings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghimmiddlings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletmiddlings milletmiddlings", () => {
  assert.ok(world.rooms.rust_milletmiddlings, "rust_milletmiddlings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletmiddlings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornmiddlings cornmiddlings", () => {
  assert.ok(world.rooms.rust_cornmiddlings, "rust_cornmiddlings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornmiddlings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatmiddlings oatmiddlings", () => {
  assert.ok(world.rooms.rust_oatmiddlings, "rust_oatmiddlings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatmiddlings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryemiddlings ryemiddlings", () => {
  assert.ok(world.rooms.rust_ryemiddlings, "rust_ryemiddlings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryemiddlings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatmiddlings wheatmiddlings", () => {
  assert.ok(world.rooms.rust_wheatmiddlings, "rust_wheatmiddlings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatmiddlings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumbran durumbran", () => {
  assert.ok(world.rooms.rust_durumbran, "rust_durumbran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumbran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniobran foniobran", () => {
  assert.ok(world.rooms.rust_foniobran, "rust_foniobran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniobran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffbran teffbran", () => {
  assert.ok(world.rooms.rust_teffbran, "rust_teffbran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffbran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybranflake barleybranflake", () => {
  assert.ok(world.rooms.rust_barleybranflake, "rust_barleybranflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybranflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbranflake sorghumbranflake", () => {
  assert.ok(world.rooms.rust_sorghumbranflake, "rust_sorghumbranflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbranflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbranflake milletbranflake", () => {
  assert.ok(world.rooms.rust_milletbranflake, "rust_milletbranflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbranflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbranflake oatbranflake", () => {
  assert.ok(world.rooms.rust_oatbranflake, "rust_oatbranflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbranflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryebranflake ryebranflake", () => {
  assert.ok(world.rooms.rust_ryebranflake, "rust_ryebranflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryebranflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatbranflake wheatbranflake", () => {
  assert.ok(world.rooms.rust_wheatbranflake, "rust_wheatbranflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatbranflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumpollard durumpollard", () => {
  assert.ok(world.rooms.rust_durumpollard, "rust_durumpollard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumpollard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniopollard foniopollard", () => {
  assert.ok(world.rooms.rust_foniopollard, "rust_foniopollard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniopollard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffpollard teffpollard", () => {
  assert.ok(world.rooms.rust_teffpollard, "rust_teffpollard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffpollard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleypollard barleypollard", () => {
  assert.ok(world.rooms.rust_barleypollard, "rust_barleypollard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleypollard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumpollard sorghumpollard", () => {
  assert.ok(world.rooms.rust_sorghumpollard, "rust_sorghumpollard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumpollard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletpollard milletpollard", () => {
  assert.ok(world.rooms.rust_milletpollard, "rust_milletpollard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletpollard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornpollard cornpollard", () => {
  assert.ok(world.rooms.rust_cornpollard, "rust_cornpollard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornpollard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatpollard oatpollard", () => {
  assert.ok(world.rooms.rust_oatpollard, "rust_oatpollard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatpollard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryepollard ryepollard", () => {
  assert.ok(world.rooms.rust_ryepollard, "rust_ryepollard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryepollard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatpollard wheatpollard", () => {
  assert.ok(world.rooms.rust_wheatpollard, "rust_wheatpollard room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatpollard"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghings sorghings", () => {
  assert.ok(world.rooms.rust_sorghings, "rust_sorghings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-millings millings", () => {
  assert.ok(world.rooms.rust_millings, "rust_millings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the millings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornings cornings", () => {
  assert.ok(world.rooms.rust_cornings, "rust_cornings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatings oatings", () => {
  assert.ok(world.rooms.rust_oatings, "rust_oatings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryeings ryeings", () => {
  assert.ok(world.rooms.rust_ryeings, "rust_ryeings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryeings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatings wheatings", () => {
  assert.ok(world.rooms.rust_wheatings, "rust_wheatings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumfeed durumfeed", () => {
  assert.ok(world.rooms.rust_durumfeed, "rust_durumfeed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumfeed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniofeed foniofeed", () => {
  assert.ok(world.rooms.rust_foniofeed, "rust_foniofeed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniofeed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tefffeed tefffeed", () => {
  assert.ok(world.rooms.rust_tefffeed, "rust_tefffeed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tefffeed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyfeed barleyfeed", () => {
  assert.ok(world.rooms.rust_barleyfeed, "rust_barleyfeed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyfeed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumfeed sorghumfeed", () => {
  assert.ok(world.rooms.rust_sorghumfeed, "rust_sorghumfeed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumfeed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletfeed milletfeed", () => {
  assert.ok(world.rooms.rust_milletfeed, "rust_milletfeed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletfeed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornfeed cornfeed", () => {
  assert.ok(world.rooms.rust_cornfeed, "rust_cornfeed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornfeed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatfeed oatfeed", () => {
  assert.ok(world.rooms.rust_oatfeed, "rust_oatfeed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatfeed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryefeed ryefeed", () => {
  assert.ok(world.rooms.rust_ryefeed, "rust_ryefeed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryefeed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatfeed wheatfeed", () => {
  assert.ok(world.rooms.rust_wheatfeed, "rust_wheatfeed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatfeed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornshorts cornshorts", () => {
  assert.ok(world.rooms.rust_cornshorts, "rust_cornshorts room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornshorts"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatmidds oatmidds", () => {
  assert.ok(world.rooms.rust_oatmidds, "rust_oatmidds room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatmidds"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryescreenings ryescreenings", () => {
  assert.ok(world.rooms.rust_ryescreenings, "rust_ryescreenings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryescreenings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatshorts wheatshorts", () => {
  assert.ok(world.rooms.rust_wheatshorts, "rust_wheatshorts room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatshorts"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryegrassseed ryegrassseed", () => {
  assert.ok(world.rooms.rust_ryegrassseed, "rust_ryegrassseed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryegrassseed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleygerm barleygerm", () => {
  assert.ok(world.rooms.rust_barleygerm, "rust_barleygerm room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleygerm"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumgerm sorghumgerm", () => {
  assert.ok(world.rooms.rust_sorghumgerm, "rust_sorghumgerm room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumgerm"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletgerm milletgerm", () => {
  assert.ok(world.rooms.rust_milletgerm, "rust_milletgerm room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletgerm"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornsemolina cornsemolina", () => {
  assert.ok(world.rooms.rust_cornsemolina, "rust_cornsemolina room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornsemolina"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatsemolina oatsemolina", () => {
  assert.ok(world.rooms.rust_oatsemolina, "rust_oatsemolina room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatsemolina"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryesemolina ryesemolina", () => {
  assert.ok(world.rooms.rust_ryesemolina, "rust_ryesemolina room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryesemolina"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatmidds wheatmidds", () => {
  assert.ok(world.rooms.rust_wheatmidds, "rust_wheatmidds room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatmidds"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbranflake cornbranflake", () => {
  assert.ok(world.rooms.rust_cornbranflake, "rust_cornbranflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbranflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-branflake branflake", () => {
  assert.ok(world.rooms.rust_branflake, "rust_branflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the branflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatflakebran oatflakebran", () => {
  assert.ok(world.rooms.rust_oatflakebran, "rust_oatflakebran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatflakebran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryeberrymeal ryeberrymeal", () => {
  assert.ok(world.rooms.rust_ryeberrymeal, "rust_ryeberrymeal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryeberrymeal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumgrits durumgrits", () => {
  assert.ok(world.rooms.rust_durumgrits, "rust_durumgrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumgrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniogrits foniogrits", () => {
  assert.ok(world.rooms.rust_foniogrits, "rust_foniogrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniogrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffgrits teffgrits", () => {
  assert.ok(world.rooms.rust_teffgrits, "rust_teffgrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffgrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-triticalegrits triticalegrits", () => {
  assert.ok(world.rooms.rust_triticalegrits, "rust_triticalegrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the triticalegrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-einkorngrits einkorngrits", () => {
  assert.ok(world.rooms.rust_einkorngrits, "rust_einkorngrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the einkorngrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-emmergrits emmergrits", () => {
  assert.ok(world.rooms.rust_emmergrits, "rust_emmergrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the emmergrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-kamutgrits kamutgrits", () => {
  assert.ok(world.rooms.rust_kamutgrits, "rust_kamutgrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the kamutgrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumflake durumflake", () => {
  assert.ok(world.rooms.rust_durumflake, "rust_durumflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumflake"), `labels: ${labels.join(" | ")}`);
});
...
test("walkthrough includes pinching the rust-kamutmeal kamutmeal", () => {
  ...
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the kamutmeal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-einkornflour einkornflour", () => {
  assert.ok(world.rooms.rust_einkornflour, "rust_einkornflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the einkornflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-emmerflour emmerflour", () => {
  assert.ok(world.rooms.rust_emmerflour, "rust_emmerflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the emmerflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-freekehflour freekehflour", () => {
  assert.ok(world.rooms.rust_freekehflour, "rust_freekehflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the freekehflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-farinagrit farinagrit", () => {
  assert.ok(world.rooms.rust_farinagrit, "rust_farinagrit room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the farinagrit"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-semolinagrit semolinagrit", () => {
  assert.ok(world.rooms.rust_semolinagrit, "rust_semolinagrit room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the semolinagrit"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatgerm oatgerm", () => {
  assert.ok(world.rooms.rust_oatgerm, "rust_oatgerm room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatgerm"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatgerm wheatgerm", () => {
  assert.ok(world.rooms.rust_wheatgerm, "rust_wheatgerm room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatgerm"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-crackedwheat crackedwheat", () => {
  assert.ok(world.rooms.rust_crackedwheat, "rust_crackedwheat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the crackedwheat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-buckwheatbran buckwheatbran", () => {
  assert.ok(world.rooms.rust_buckwheatbran, "rust_buckwheatbran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the buckwheatbran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-amaranthflour amaranthflour", () => {
  assert.ok(world.rooms.rust_amaranthflour, "rust_amaranthflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the amaranthflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-quinoaflour quinoaflour", () => {
  assert.ok(world.rooms.rust_quinoaflour, "rust_quinoaflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the quinoaflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniomeal foniomeal", () => {
  assert.ok(world.rooms.rust_foniomeal, "rust_foniomeal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniomeal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffmeal teffmeal", () => {
  assert.ok(world.rooms.rust_teffmeal, "rust_teffmeal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffmeal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-fonioflour fonioflour", () => {
  assert.ok(world.rooms.rust_fonioflour, "rust_fonioflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the fonioflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffflour teffflour", () => {
  assert.ok(world.rooms.rust_teffflour, "rust_teffflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-triticaleberry triticaleberry", () => {
  assert.ok(world.rooms.rust_triticaleberry, "rust_triticaleberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the triticaleberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-einkornberry einkornberry", () => {
  assert.ok(world.rooms.rust_einkornberry, "rust_einkornberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the einkornberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-emmerberry emmerberry", () => {
  assert.ok(world.rooms.rust_emmerberry, "rust_emmerberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the emmerberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-kamutberry kamutberry", () => {
  assert.ok(world.rooms.rust_kamutberry, "rust_kamutberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the kamutberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-maizegroat maizegroat", () => {
  assert.ok(world.rooms.rust_maizegroat, "rust_maizegroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the maizegroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumgroat durumgroat", () => {
  assert.ok(world.rooms.rust_durumgroat, "rust_durumgroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumgroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-foniogroat foniogroat", () => {
  assert.ok(world.rooms.rust_foniogroat, "rust_foniogroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the foniogroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-teffgroat teffgroat", () => {
  assert.ok(world.rooms.rust_teffgroat, "rust_teffgroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the teffgroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatgroat wheatgroat", () => {
  assert.ok(world.rooms.rust_wheatgroat, "rust_wheatgroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatgroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatgroat oatgroat", () => {
  assert.ok(world.rooms.rust_oatgroat, "rust_oatgroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatgroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleygroat barleygroat", () => {
  assert.ok(world.rooms.rust_barleygroat, "rust_barleygroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleygroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumgroat sorghumgroat", () => {
  assert.ok(world.rooms.rust_sorghumgroat, "rust_sorghumgroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumgroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletgroat milletgroat", () => {
  assert.ok(world.rooms.rust_milletgroat, "rust_milletgroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletgroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-triticalegroat triticalegroat", () => {
  assert.ok(world.rooms.rust_triticalegroat, "rust_triticalegroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the triticalegroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-emmergroat emmergroat", () => {
  assert.ok(world.rooms.rust_emmergroat, "rust_emmergroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the emmergroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-einkorngroat einkorngroat", () => {
  assert.ok(world.rooms.rust_einkorngroat, "rust_einkorngroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the einkorngroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-freekehgroat freekehgroat", () => {
  assert.ok(world.rooms.rust_freekehgroat, "rust_freekehgroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the freekehgroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-kamutgroat kamutgroat", () => {
  assert.ok(world.rooms.rust_kamutgroat, "rust_kamutgroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the kamutgroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-speltgroat speltgroat", () => {
  assert.ok(world.rooms.rust_speltgroat, "rust_speltgroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the speltgroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-triticaleflake triticaleflake", () => {
  assert.ok(world.rooms.rust_triticaleflake, "rust_triticaleflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the triticaleflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-emmerflake emmerflake", () => {
  assert.ok(world.rooms.rust_emmerflake, "rust_emmerflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the emmerflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-einkornflake einkornflake", () => {
  assert.ok(world.rooms.rust_einkornflake, "rust_einkornflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the einkornflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-freekehflake freekehflake", () => {
  assert.ok(world.rooms.rust_freekehflake, "rust_freekehflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the freekehflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-kamutflake kamutflake", () => {
  assert.ok(world.rooms.rust_kamutflake, "rust_kamutflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the kamutflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryeflake ryeflake", () => {
  assert.ok(world.rooms.rust_ryeflake, "rust_ryeflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryeflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyflake barleyflake", () => {
  assert.ok(world.rooms.rust_barleyflake, "rust_barleyflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumflake sorghumflake", () => {
  assert.ok(world.rooms.rust_sorghumflake, "rust_sorghumflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletflake milletflake", () => {
  assert.ok(world.rooms.rust_milletflake, "rust_milletflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-riceflake riceflake", () => {
  assert.ok(world.rooms.rust_riceflake, "rust_riceflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the riceflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornflake cornflake", () => {
  assert.ok(world.rooms.rust_cornflake, "rust_cornflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatflake wheatflake", () => {
  assert.ok(world.rooms.rust_wheatflake, "rust_wheatflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatflake oatflake", () => {
  assert.ok(world.rooms.rust_oatflake, "rust_oatflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-speltflake speltflake", () => {
  assert.ok(world.rooms.rust_speltflake, "rust_speltflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the speltflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-groatflake groatflake", () => {
  assert.ok(world.rooms.rust_groatflake, "rust_groatflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the groatflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-farroberry farroberry", () => {
  assert.ok(world.rooms.rust_farroberry, "rust_farroberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the farroberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyberry barleyberry", () => {
  assert.ok(world.rooms.rust_barleyberry, "rust_barleyberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumberry sorghumberry", () => {
  assert.ok(world.rooms.rust_sorghumberry, "rust_sorghumberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletberry milletberry", () => {
  assert.ok(world.rooms.rust_milletberry, "rust_milletberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-riceberry riceberry", () => {
  assert.ok(world.rooms.rust_riceberry, "rust_riceberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the riceberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornberry cornberry", () => {
  assert.ok(world.rooms.rust_cornberry, "rust_cornberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatberry wheatberry", () => {
  assert.ok(world.rooms.rust_wheatberry, "rust_wheatberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatberry oatberry", () => {
  assert.ok(world.rooms.rust_oatberry, "rust_oatberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryeberry ryeberry", () => {
  assert.ok(world.rooms.rust_ryeberry, "rust_ryeberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryeberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-pumpkinflour pumpkinflour", () => {
  assert.ok(world.rooms.rust_pumpkinflour, "rust_pumpkinflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the pumpkinflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-hempflour hempflour", () => {
  assert.ok(world.rooms.rust_hempflour, "rust_hempflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the hempflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-chiaflour chiaflour", () => {
  assert.ok(world.rooms.rust_chiaflour, "rust_chiaflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the chiaflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-nigellaflour nigellaflour", () => {
  assert.ok(world.rooms.rust_nigellaflour, "rust_nigellaflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the nigellaflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-poppyflour poppyflour", () => {
  assert.ok(world.rooms.rust_poppyflour, "rust_poppyflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the poppyflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sunflowerflour sunflowerflour", () => {
  assert.ok(world.rooms.rust_sunflowerflour, "rust_sunflowerflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sunflowerflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sesameflour sesameflour", () => {
  assert.ok(world.rooms.rust_sesameflour, "rust_sesameflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sesameflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleybran barleybran", () => {
  assert.ok(world.rooms.rust_barleybran, "rust_barleybran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleybran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumbran sorghumbran", () => {
  assert.ok(world.rooms.rust_sorghumbran, "rust_sorghumbran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumbran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletbran milletbran", () => {
  assert.ok(world.rooms.rust_milletbran, "rust_milletbran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletbran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-speltberry speltberry", () => {
  assert.ok(world.rooms.rust_speltberry, "rust_speltberry room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the speltberry"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-buckwheatgroat buckwheatgroat", () => {
  assert.ok(world.rooms.rust_buckwheatgroat, "rust_buckwheatgroat room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the buckwheatgroat"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-quinoaflake quinoaflake", () => {
  assert.ok(world.rooms.rust_quinoaflake, "rust_quinoaflake room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the quinoaflake"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-amaranthseed amaranthseed", () => {
  assert.ok(world.rooms.rust_amaranthseed, "rust_amaranthseed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the amaranthseed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-farinaflour farinaflour", () => {
  assert.ok(world.rooms.rust_farinaflour, "rust_farinaflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the farinaflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-coconutflour coconutflour", () => {
  assert.ok(world.rooms.rust_coconutflour, "rust_coconutflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the coconutflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-almondflour almondflour", () => {
  assert.ok(world.rooms.rust_almondflour, "rust_almondflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the almondflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-peanutflour peanutflour", () => {
  assert.ok(world.rooms.rust_peanutflour, "rust_peanutflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the peanutflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-lentilflour lentilflour", () => {
  assert.ok(world.rooms.rust_lentilflour, "rust_lentilflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the lentilflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-chickpeaflour chickpeaflour", () => {
  assert.ok(world.rooms.rust_chickpeaflour, "rust_chickpeaflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the chickpeaflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-buckwheatflour buckwheatflour", () => {
  assert.ok(world.rooms.rust_buckwheatflour, "rust_buckwheatflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the buckwheatflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ricebran ricebran", () => {
  assert.ok(world.rooms.rust_ricebran, "rust_ricebran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ricebran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornbran cornbran", () => {
  assert.ok(world.rooms.rust_cornbran, "rust_cornbran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornbran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatbran wheatbran", () => {
  assert.ok(world.rooms.rust_wheatbran, "rust_wheatbran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatbran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatbran oatbran", () => {
  assert.ok(world.rooms.rust_oatbran, "rust_oatbran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatbran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryebran ryebran", () => {
  assert.ok(world.rooms.rust_ryebran, "rust_ryebran room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryebran"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-speltflour speltflour", () => {
  assert.ok(world.rooms.rust_speltflour, "rust_speltflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the speltflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-durumflour durumflour", () => {
  assert.ok(world.rooms.rust_durumflour, "rust_durumflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the durumflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-breadflour breadflour", () => {
  assert.ok(world.rooms.rust_breadflour, "rust_breadflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the breadflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-pastryflour pastryflour", () => {
  assert.ok(world.rooms.rust_pastryflour, "rust_pastryflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the pastryflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-grahamflour grahamflour", () => {
  assert.ok(world.rooms.rust_grahamflour, "rust_grahamflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the grahamflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-arrowroot arrowroot", () => {
  assert.ok(world.rooms.rust_arrowroot, "rust_arrowroot room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the arrowroot"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sago sago", () => {
  assert.ok(world.rooms.rust_sago, "rust_sago room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sago"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-tapioca tapioca", () => {
  assert.ok(world.rooms.rust_tapioca, "rust_tapioca room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the tapioca"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cassava cassava", () => {
  assert.ok(world.rooms.rust_cassava, "rust_cassava room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cassava"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-riceflour riceflour", () => {
  assert.ok(world.rooms.rust_riceflour, "rust_riceflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the riceflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornflour cornflour", () => {
  assert.ok(world.rooms.rust_cornflour, "rust_cornflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatflour wheatflour", () => {
  assert.ok(world.rooms.rust_wheatflour, "rust_wheatflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleyflour barleyflour", () => {
  assert.ok(world.rooms.rust_barleyflour, "rust_barleyflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleyflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatflour oatflour", () => {
  assert.ok(world.rooms.rust_oatflour, "rust_oatflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryeflour ryeflour", () => {
  assert.ok(world.rooms.rust_ryeflour, "rust_ryeflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryeflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletflour milletflour", () => {
  assert.ok(world.rooms.rust_milletflour, "rust_milletflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumflour sorghumflour", () => {
  assert.ok(world.rooms.rust_sorghumflour, "rust_sorghumflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-freekeh freekeh", () => {
  assert.ok(world.rooms.rust_freekeh, "rust_freekeh room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the freekeh"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-kamut kamut", () => {
  assert.ok(world.rooms.rust_kamut, "rust_kamut room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the kamut"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sticky sticky", () => {
  assert.ok(world.rooms.rust_sticky, "rust_sticky room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sticky"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-blackrice blackrice", () => {
  assert.ok(world.rooms.rust_blackrice, "rust_blackrice room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the blackrice"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wildrice wildrice", () => {
  assert.ok(world.rooms.rust_wildrice, "rust_wildrice room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wildrice"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-redrice redrice", () => {
  assert.ok(world.rooms.rust_redrice, "rust_redrice room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the redrice"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-jasmine jasmine", () => {
  assert.ok(world.rooms.rust_jasmine, "rust_jasmine room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the jasmine"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-basmati basmati", () => {
  assert.ok(world.rooms.rust_basmati, "rust_basmati room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the basmati"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-glutinous glutinous", () => {
  assert.ok(world.rooms.rust_glutinous, "rust_glutinous room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the glutinous"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-japonica japonica", () => {
  assert.ok(world.rooms.rust_japonica, "rust_japonica room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the japonica"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-chia chia", () => {
  assert.ok(world.rooms.rust_chia, "rust_chia room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the chia"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-hempseed hempseed", () => {
  assert.ok(world.rooms.rust_hempseed, "rust_hempseed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the hempseed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-pumpkinseed pumpkinseed", () => {
  assert.ok(world.rooms.rust_pumpkinseed, "rust_pumpkinseed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the pumpkinseed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-flaxseed flaxseed", () => {
  assert.ok(world.rooms.rust_flaxseed, "rust_flaxseed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the flaxseed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-nigella nigella", () => {
  assert.ok(world.rooms.rust_nigella, "rust_nigella room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the nigella"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-poppyseed poppyseed", () => {
  assert.ok(world.rooms.rust_poppyseed, "rust_poppyseed room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the poppyseed"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sunflower sunflower", () => {
  assert.ok(world.rooms.rust_sunflower, "rust_sunflower room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sunflower"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sesame sesame", () => {
  assert.ok(world.rooms.rust_sesame, "rust_sesame room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sesame"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-vialone vialone", () => {
  assert.ok(world.rooms.rust_vialone, "rust_vialone room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the vialone"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-carnaroli carnaroli", () => {
  assert.ok(world.rooms.rust_carnaroli, "rust_carnaroli room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the carnaroli"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-arborio arborio", () => {
  assert.ok(world.rooms.rust_arborio, "rust_arborio room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the arborio"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-orzo orzo", () => {
  assert.ok(world.rooms.rust_orzo, "rust_orzo room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the orzo"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-polentameal polentameal", () => {
  assert.ok(world.rooms.rust_polentameal, "rust_polentameal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the polentameal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-semolinaflour semolinaflour", () => {
  assert.ok(world.rooms.rust_semolinaflour, "rust_semolinaflour room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the semolinaflour"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleygrits barleygrits", () => {
  assert.ok(world.rooms.rust_barleygrits, "rust_barleygrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleygrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatgrits wheatgrits", () => {
  assert.ok(world.rooms.rust_wheatgrits, "rust_wheatgrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatgrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatgrits oatgrits", () => {
  assert.ok(world.rooms.rust_oatgrits, "rust_oatgrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatgrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryegrits ryegrits", () => {
  assert.ok(world.rooms.rust_ryegrits, "rust_ryegrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryegrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghumgrits sorghumgrits", () => {
  assert.ok(world.rooms.rust_sorghumgrits, "rust_sorghumgrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghumgrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletgrits milletgrits", () => {
  assert.ok(world.rooms.rust_milletgrits, "rust_milletgrits room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletgrits"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-steelcutoats steelcutoats", () => {
  assert.ok(world.rooms.rust_steelcutoats, "rust_steelcutoats room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the steelcutoats"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-farina farina", () => {
  assert.ok(world.rooms.rust_farina, "rust_farina room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the farina"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-bulgur bulgur", () => {
  assert.ok(world.rooms.rust_bulgur, "rust_bulgur room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the bulgur"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-couscous couscous", () => {
  assert.ok(world.rooms.rust_couscous, "rust_couscous room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the couscous"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-kasha kasha", () => {
  assert.ok(world.rooms.rust_kasha, "rust_kasha room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the kasha"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-rolledoats rolledoats", () => {
  assert.ok(world.rooms.rust_rolledoats, "rust_rolledoats room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the rolledoats"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-pearlbarley pearlbarley", () => {
  assert.ok(world.rooms.rust_pearlbarley, "rust_pearlbarley room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the pearlbarley"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-groats groats", () => {
  assert.ok(world.rooms.rust_groats, "rust_groats room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the groats"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-middlings middlings", () => {
  assert.ok(world.rooms.rust_middlings, "rust_middlings room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the middlings"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-grist grist", () => {
  assert.ok(world.rooms.rust_grist, "rust_grist room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the grist"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sampmeal sampmeal", () => {
  assert.ok(world.rooms.rust_sampmeal, "rust_sampmeal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sampmeal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-nixtamal nixtamal", () => {
  assert.ok(world.rooms.rust_nixtamal, "rust_nixtamal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the nixtamal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-masa masa", () => {
  assert.ok(world.rooms.rust_masa, "rust_masa room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the masa"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-sorghummeal sorghummeal", () => {
  assert.ok(world.rooms.rust_sorghummeal, "rust_sorghummeal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the sorghummeal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-milletmeal milletmeal", () => {
  assert.ok(world.rooms.rust_milletmeal, "rust_milletmeal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the milletmeal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-wheatmeal wheatmeal", () => {
  assert.ok(world.rooms.rust_wheatmeal, "rust_wheatmeal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the wheatmeal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-barleymeal barleymeal", () => {
  assert.ok(world.rooms.rust_barleymeal, "rust_barleymeal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the barleymeal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-ryegrain ryegrain", () => {
  assert.ok(world.rooms.rust_ryegrain, "rust_ryegrain room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the ryegrain"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-oatmeal oatmeal", () => {
  assert.ok(world.rooms.rust_oatmeal, "rust_oatmeal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the oatmeal"), `labels: ${labels.join(" | ")}`);
});

test("walkthrough includes pinching the rust-cornmeal cornmeal", () => {
  assert.ok(world.rooms.rust_cornmeal, "rust_cornmeal room missing");
  const labels = world.walkthrough.filter((w): w is string => typeof w === "string");
  assert.ok(labels.includes("pinch the cornmeal"), `labels: ${labels.join(" | ")}`);
});
